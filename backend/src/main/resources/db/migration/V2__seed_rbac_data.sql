...
-- Step 6: Create initial HR user and assign ROLE_HR
-- Password will use the same BCrypt hash pattern as admin (change if needed)
INSERT INTO users (
    username, 
    email, 
    password, 
    ...
    'hr.user',
    'hr@example.com',
    -- BCrypt hash (same rounds=10) — replace if you want a different password
    '$2a$10$CMma736Zxup0lwfPCPvsQOxzrZR6xqm30KDgn1fdMwIbBskcsjYum',
    TRUE,
    TRUE,
    TRUE,
...
-- Assign ROLE_HR to the HR user
INSERT INTO user_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u
CROSS JOIN roles r
WHERE u.username = 'hr.user'
AND r.name = 'ROLE_HR';

...
public interface PositionRepository extends JpaRepository<Position, Long> {
     ...
     */
    @Query("SELECT p FROM Position p JOIN FETCH p.department LEFT JOIN FETCH p.officeLocation WHERE p.isActive = true")
    List<Position> findAllActiveWithDepartment();

    /**
     * Find all active positions belonging to a specific department
     */
    @Query("SELECT p FROM Position p JOIN FETCH p.department d LEFT JOIN FETCH p.officeLocation WHERE p.isActive = true AND d.id = :departmentId")

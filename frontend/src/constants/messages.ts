// src/constants/messages.ts
export const SYSTEM_MESSAGES = {
  // API & Load States
  API_ERROR: "Không thể tải dữ liệu. Vui lòng thử lại.",
  LOADING: "Đang tải dữ liệu…",
  VIEW_MODE: "Chế độ xem",
  NO_DATA: "Không có dữ liệu",
  DOWNLOAD_TOOLTIP: "Tải xuống",
  SEARCH_PLACEHOLDER: "Tìm kiếm...",
  SELECT_PLACEHOLDER: "Chọn giá trị",
  SUCCESS_UPDATE: "Cập nhật thành công!",
  ERROR: "Có lỗi xảy ra",
  SAVING: "Đang lưu...",
  AUTH_LOADING: "Đang xác thực...",
  LOADING_SHORT: "Đang tải...",
  SELECT_TYPE: "Chọn phân loại",
  SELECT_STATUS: "Chọn trạng thái",
  SEARCH: "Tìm kiếm...",
  CURRENCY_SUFFIX: " đ",
  // Actions
  BTN_CANCEL: "Hủy bỏ",
  BTN_UPDATE: "Cập nhật",
  BTN_SAVE: "Lưu thay đổi",
  BTN_DELETE: "Xóa",
  BTN_ADD: "Thêm mới",
  BTN_EDIT: "Chỉnh sửa",
  BTN_CLOSE: "Đóng",
  DIALOG: {
    TITLE: "Hộp thoại",
    DESCRIPTION: "Chi tiết nội dung hộp thoại",
    CLOSE: "Đóng",
  },
  BTN_CONFIRM: "Xác nhận",
  BTN_SUBMIT: "Gửi",
  BTN_BACK: "Quay lại",
  // Common labels
  LABEL_STATUS: "Trạng thái",
  LABEL_DATE: "Ngày",
  LABEL_TIME: "Thời gian",
  LABEL_EMPLOYEE: "Nhân viên",
  LABEL_DEPARTMENT: "Phòng ban",
  LABEL_ROLE: "Vị trí",
  LABEL_REASON: "Lý do",
  LABEL_DESC: "Mô tả",
  LABEL_CREATED_AT: "Ngày tạo",
  LABEL_ACTION: "Thao tác",
  LABEL_ALL: "Tất cả",
  // Sidebar & Navigation
  SIDEBAR: {
    ADMIN_SECTION: "Quản lý hệ thống",
    MANAGER_SECTION: "Quản lý đội nhóm",
    HR_SECTION: "Quản lý nhân sự",
    EMPLOYEE_SECTION: "Thông tin cá nhân",
    MENU_PROFILE: "Hồ sơ của tôi",
    MENU_EMP_MGMT: "Quản lý nhân viên",
    MENU_ASSET_MGMT: "Quản lý tài sản",
    MENU_APPROVE_LEAVE: "Duyệt đơn nghỉ phép",
    MENU_APPROVE_ADJ: "Duyệt điều chỉnh chấm công",
    MENU_GROUP_ASSET: "Tài sản nhóm",
    MENU_CHECKIN: "Chấm công",
    MENU_REQUEST: "Tạo đơn nghỉ phép",
    MENU_ATTENDANCE_SETTINGS: "Cấu hình chấm công",
    MENU_POLICY_SETTINGS: "Cấu hình chính sách",
    MENU_RUN_PAYROLL: "Chạy tính lương",
    MENU_MY_ASSETS: "Tài sản của tôi",
    MENU_MEMBERS: "Thành viên nhóm",
    MENU_KPI: "Thiết lập KPI/OKR",
    MENU_PAYROLL: "Quản lý lương",
    MENU_AUDIT_LOGS: "Nhật ký kiểm toán",
    MENU_ASSET_INCIDENT: "Quản lý sự cố tài sản",
    MENU_APPROVE_REPORTS: "Duyệt báo cáo sự cố",
    MENU_APPROVE_ASSET_REQUESTS: "Duyệt yêu cầu tài sản",
    MENU_HR_PAYROLL_PERIOD: "Bảng lương theo kì",
    MENU_ATTENDANCE: "Lịch sử điểm danh",
    MENU_DASHBOARD: "Tổng quan",
    MENU_ANNOUNCEMENTS: "Thông báo nội bộ",
    MENU_MANAGE_ANNOUNCEMENTS: "Tạo thông báo",
    MENU_SALARY_HISTORY: "Lịch sử lương",
    LOGOUT: "Đăng xuất",
    LOGGING_OUT: "Đang đăng xuất...",
    HEADER_TITLE: "Menu",
    QUICK_CREATE: "Tạo nhanh",
    INBOX: "Hộp thư",
    DOCUMENTS: "HRMS",
    MORE: "Thêm",
    OPEN: "Mở",
    SHARE: "Chia sẻ",
    HRMS: "HRMS",
    VERSION_PREFIX: "v",
    SECTION_CREATE: "Tạo đơn",
    SECTION_APPROVE: "Duyệt đơn",
    SECTION_PERSONAL: "Cá nhân",
    SECTION_MANAGEMENT: "Quản lý",
    LABEL_SIDEBAR: "Sidebar",
    DESC_MOBILE: "Displays the mobile sidebar.",
    TOGGLE_TOOLTIP: "Toggle Sidebar",
  },
  // Profile Page
  PROFILE: {
    BREADCRUMB: "Cổng thông tin > Hồ sơ của tôi",
    TITLE: "Thông tin cá nhân",
    MODAL_SUCCESS: "Cập nhật thông tin thành công!",
    DEPARTMENT_PREFIX: "Phòng ",
    OFFICE_LOCATION: "Văn phòng Hà Nội",
    EMP_CODE: "Mã nhân viên",
    MANAGER: "Quản lý",
    NO_MANAGER: "Chưa có quản lý",
    CONTACT_SECTION: "Thông tin liên hệ & Cá nhân",
    FULL_NAME: "Họ và tên",
    EMAIL: "Email công ty",
    NATIONAL_ID: "CCCD/CMND",
    PHONE: "Số điện thoại (Tùy chọn)",
    DOB: "Ngày sinh",
    SELECT_DATE: "Chọn ngày",
    JOB_SECTION: "Thông tin công việc",
    WORK_STATUS: "Trạng thái làm việc",
    CONTRACT: "Loại hợp đồng",
    START_DATE: "Ngày bắt đầu",
    END_DATE: "Ngày kết thúc (Tùy chọn)",
    DOCS_SECTION: "Tài liệu của tôi",
    STATS_TITLE: "Thống kê nhanh",
    STATS_LEAVE: "Ngày phép còn lại",
    STATS_ATTENDANCE: "Chuyên cần",
    PERCENT_SIGN: "%",
    NAME_PLACEHOLDER: "Nhập họ và tên",
    EMAIL_PLACEHOLDER: "email@company.com",
    ID_PLACEHOLDER: "012345678912",
    PHONE_PLACEHOLDER: "0912345678",
    LOADING: "Đang xác thực...",
    SUCCESS: "Đăng nhập thành công!",
  },
  // Attendance History
  ATTENDANCE_HIST: {
    BREADCRUMB: "Chấm công",
    TITLE: "Lịch sử điểm danh",
    DESC: "Xem toàn bộ lịch sử chấm công của bạn",
    CALENDAR_TITLE: "Lịch chấm công theo tháng",
    DETAIL_TITLE: "Chi tiết chấm công",
    DETAIL_EMPTY: "Chọn một ngày trên lịch để xem chi tiết điểm danh",
    CARD_WORK_DAYS: "Ngày đi làm",
    CARD_FULL_WORK_DAYS: "Số ngày đủ công",
    CARD_LATE_DAYS: "Ngày đi muộn",
    CARD_NO_CLOCK_OUT_DAYS: "Không Clock Out",
    CARD_ABSENT_DAYS: "Ngày vắng mặt",
    CARD_TOTAL_HOURS: "Tổng giờ làm",
    COMPARE_PREV_MONTH: "so với tháng trước",
    CARD_MONTH_LABEL: (month: string) => `Tháng ${month}`,
    TABLE_TITLE: "Chi tiết chấm công",
    FILTER_STATUS_PLACEHOLDER: "Lọc trạng thái",
    UNIT_RECORDS: "bản ghi",
    TOTAL_PREFIX: "Tổng",
    LABEL_DATE: "Ngày",
    LABEL_STATUS: "Trạng thái",
    LABEL_CHECKIN: "Check-in",
    LABEL_CHECKOUT: "Check-out",
    LABEL_TOTAL_WORK_HOURS: "Tổng giờ làm",
    LABEL_MISSING_CLOCKOUT: "Bản ghi này chưa có Clock Out.",
    LABEL_NOTES: "Ghi chú",
  },
  // Salary History
  SALARY_HISTORY: {
    TITLE: "Lịch sử kỳ lương",
    DESC: "Theo dõi chi tiết các khoản thu nhập và khấu trừ hàng tháng của bạn.",
    LABEL_YEAR: "Năm",
    TABLE_PERIOD: "Kỳ lương",
    TABLE_PAYMENT_DATE: "Ngày thanh toán",
    TABLE_TOTAL_INCOME: "Tổng thu nhập",
    TABLE_DEDUCTIONS: "Khấu trừ",
    TABLE_NET_PAY: "Thực lĩnh (Net)",
    PAGINATION_SHOW: "Hiển thị",
    PAGINATION_ON_TOTAL: "trên tổng số",
    PAGINATION_PERIODS: "kỳ lương",
    STATS_TOTAL_INCOME: "Tổng thu nhập năm",
    STATS_AVG_NET: "Trung bình Thực lĩnh/tháng",
    STATS_NEXT_PERIOD: "Kỳ lương tiếp theo",
    SHEET_TITLE: "Chi tiết phiếu lương",
    SHEET_DESC: "Xem chi tiết các khoản lương và khấu trừ trong kỳ.",
    SHEET_PERIOD: "Kỳ lương",
    SHEET_PAYMENT_DATE: "Ngày thanh toán",
    SHEET_BASE_SALARY: "Lương cơ bản",
    SHEET_ALLOWANCES: "Phụ cấp",
    SHEET_TOTAL_ALLOWANCES: "Tổng phụ cấp",
    SHEET_DEDUCTIONS: "Khấu trừ",
    SHEET_TOTAL_DEDUCTIONS: "Tổng khấu trừ",
    SHEET_NET_PAY: "Tổng thực lĩnh",
    SHEET_DOWNLOAD: "Tải phiếu lương (PDF)",
    SHEET_CLOSE: "Đóng",
    PLACEHOLDER_MONTH: "Chọn tháng",
    PLACEHOLDER_STATUS: "Trạng thái",
    LABEL_SALARY: "Lương",
    BTN_SAVE: "Lưu",
    LABEL_PAYMENT_METHOD: "Phương thức",
    LABEL_METHOD_SHORT: "Phương thức:",
    LABEL_REFERENCE_CODE: "Mã tham chiếu",
    LABEL_REF_SHORT: "Ref:",
    STATUS_PAID: "Đã thanh toán",
    STATUS_PAID_CAP: "ĐÃ THANH TOÁN",
    STATUS_PENDING: "Chờ thanh toán",
    STATUS_PENDING_CAP: "CHỜ THANH TOÁN",
    BTN_ADD_ALLOWANCE: "Thêm phụ cấp",
    BTN_ADD_DEDUCTION: "Thêm khấu trừ",
    LOADING_SALARY: "Đang tải dữ liệu lương...",
    ERROR_LOAD_SALARY: "Không thể tải dữ liệu. Vui lòng thử lại.",
    NO_DATA: "Không có dữ liệu",
    ALL_YEARS: "Tất cả năm",
    ALL_MONTHS: "Tất cả tháng",
    MONTH_PREFIX: "Tháng",
    ALL_STATUS: "Tất cả trạng thái",
    NEXT_PAYMENT_DATE: "05/06/2026",
    // PDF Template Strings
    PDF_TITLE: "PHIẾU LƯƠNG",
    PDF_LABEL_EMPLOYEE: "Nhân viên",
    PDF_LABEL_CODE: "Mã NV",
    PDF_LABEL_DEPT: "Phòng ban",
    PDF_LABEL_DATE: "Ngày thanh toán",
    PDF_LABEL_EXPORT_DATE: "Ngày xuất",
    PDF_LABEL_INCOME_DETAIL: "Chi tiết thu nhập",
    PDF_LABEL_DEDUCTION_DETAIL: "Các khoản khấu trừ",
    PDF_LABEL_TOTAL_INCOME: "TỔNG THU NHẬP",
    PDF_LABEL_TOTAL_DEDUCTION: "TỔNG KHẤU TRỪ",
    PDF_LABEL_NET_PAY: "THỰC LĨNH (NET)",
    PDF_FOOTER_THANKS: "Cảm ơn bạn vì những đóng góp cho công ty!",
    PDF_FOOTER_AUTO: "Đây là phiếu lương tự động, không cần chữ ký.",
  },

  // Check-in Page
  CHECKIN: {
    BREADCRUMB: "Cổng thông tin > Chấm công",
    TITLE: "Điểm danh nhân viên",
    DESC: "Quản lý thời gian làm việc và lịch sử chấm công của bạn.",
    STATUS_NOT_CHECKED: "Chưa điểm danh",
    STATUS_WORKING: (time: string) => `Đang làm việc (Vào lúc ${time})`,
    STATUS_CHECKED_OUT: (inTime: string, outTime: string) =>
      `Đã Check-out (Vào: ${inTime} — Ra: ${outTime})`,
    MSG_MORNING: "Chào buổi sáng! Hãy bắt đầu ngày làm việc đầy năng lượng.",
    MSG_WORKING: "Bạn đang trong ca làm việc. Chúc một ngày làm việc hiệu quả!",
    MSG_DONE: "Bạn đã kết thúc ca làm việc hôm nay. Nghỉ ngơi tốt nhé!",
    BTN_CHECKIN: "Check In Ngay",
    BTN_CHECKOUT: "Check Out",
    BTN_ADJUST: "Điều chỉnh chấm công",
    WORK_HOURS: "Tổng giờ làm",
    LATE_DAYS: "Số ngày đi muộn",
    ATTENDANCE_RATE: "Tỷ lệ chuyên cần",
    THIS_MONTH: "Tháng này",
    NO_DATA: "Chưa có dữ liệu chấm công.",
    NO_DATA_SHORT: "—",
    HISTORY_TITLE: "Lịch sử điểm danh (7 ngày gần nhất)",
    TABLE_DATE: "Ngày",
    TABLE_CHECKIN: "Check-in",
    TABLE_CHECKOUT: "Check-out",
    TABLE_TOTAL_HOURS: "Tổng giờ",
    TABLE_STATUS: "Trạng thái",
    VIEW_ALL: "Xem tất cả",
    CONFIRM_IN: "Xác nhận Check-in",
    CONFIRM_OUT: "Xác nhận Check-out",
    CAMERA_DESC: "Chụp ảnh khuôn mặt và xác nhận vị trí của bạn để điểm danh.",
    STATUS_PRESENT: "Đúng giờ",
    STATUS_LATE: "Đi muộn",
    STATUS_ABSENT: "Vắng",
    STATUS_HALF: "Nửa ngày",
    STATUS_ON_LEAVE: "Nghỉ phép",
    DONE_CHECKOUT: "Đã hoàn thành Check-out",
    CHECKIN_POLICY_DETAIL: "Chi tiết chính sách chấm công",
  },
  // Manager - Approve Adjustments
  MGMT_ADJ: {
    BREADCRUMB_PARENT: "Phê duyệt",
    BREADCRUMB_CURRENT: "Điều chỉnh chấm công",
    TITLE: "Danh sách chờ duyệt",
    DESC: "Quản lý các yêu cầu điều chỉnh chấm công từ nhân viên.",
    PENDING_STATS_LABEL: "Yêu cầu chờ duyệt",
    SEARCH_PLACEHOLDER: "Tìm nhân viên hoặc mã...",
    FILTER_STATUS: "Trạng thái",
    BTN_CLEAR_FILTER: "Xóa bộ lọc",
    BTN_CUSTOM_DATE: "Tùy chỉnh ngày",
    BTN_EXPORT: "Xuất báo cáo",
    TABLE_EMP: "Nhân viên",
    TABLE_ADJ_DATE: "Ngày điều chỉnh",
    TABLE_TYPE: "Loại chấm công",
    TABLE_STATUS: "Trạng thái",
    TABLE_ACTIONS: "Thao tác",
    EMPTY_DATA: "Không có dữ liệu",
    BTN_APPROVE: "Duyệt",
    BTN_REJECT: "Từ chối",
    BTN_DETAIL: "Xem chi tiết",
    SUMMARY_TOTAL: "Tổng",
    SUMMARY_UNIT: "yêu cầu",
    MSG_FETCH_ERROR: "Không thể tải danh sách yêu cầu.",
    MSG_APPROVE_SUCCESS: "Đã duyệt yêu cầu thành công!",
    MSG_REJECT_SUCCESS: "Đã từ chối yêu cầu.",
    MSG_RETURN_SUCCESS: "Đã trả yêu cầu về nhân viên.",
  },
  // Status Labels (Generic)
  STATUS: {
    PENDING: "Chờ duyệt",
    APPROVED: "Đã duyệt",
    REJECTED: "Từ chối",
    RETURNED: "Trả về",
    CANCELLED: "Đã hủy",
    ACTIVE: "Hoạt động",
    INACTIVE: "Không hoạt động",
    SUSPENDED: "Đình chỉ",
    PRESENT: "Đúng giờ",
    LATE: "Đi muộn",
    ABSENT: "Vắng mặt",
    HALF_DAY: "Nửa ngày",
    ON_LEAVE: "Nghỉ phép",
    AVAILABLE: "Sẵn dùng",
    ASSIGNED: "Đang cấp phát",
    RETURNED_ASSET: "Đã thu hồi",
    NEW: "Mới",
    GOOD: "Tốt",
    FAIR: "Trung bình",
    NEED_REPAIR: "Cần sửa chữa",
    NORMAL: "Bình thường",
    MAINTENANCE: "Bảo trì",
    CHECKED_IN: "Đã checkin",
    NOT_CHECKED_IN: "Chưa checkin",
    UNKNOWN: "Không xác định",
  },
  // Leave Request
  LEAVE: {
    EMPTY_FILTER_TITLE: "Không tìm thấy kết quả",
    EMPTY_FILTER_DESC: "Hãy thử thay đổi hoặc xóa bộ lọc để xem kết quả.",
    EMPTY_TITLE: "Chưa có đơn nghỉ phép",
    EMPTY_DESC: "Bạn chưa tạo đơn xin nghỉ phép nào.",
    BREADCRUMB: "Chấm công / Đơn nghỉ phép",
    TITLE: "Đơn xin nghỉ phép",
    DESC: "Quản lý và theo dõi trạng thái các đơn xin nghỉ phép của bạn.",
    BTN_CREATE: "Tạo đơn mới",
    FILTER_STATUS: "Trạng thái",
    FILTER_TYPE: "Loại phép",
    BTN_CLEAR: "Xóa bộ lọc",
    TABLE_ID: "Mã đơn",
    TABLE_DATE_CREATED: "Ngày tạo",
    TABLE_DATE_LEAVE: "Thời gian nghỉ",
    TABLE_TYPE: "Loại phép",
    TABLE_STATUS: "Trạng thái",
    DAYS: "ngày",
    BTN_DETAIL: "Xem chi tiết",
    BTN_RESEND: "Gửi lại",
    BTN_CANCEL: "Hủy đơn",
    SUMMARY_SHOW: "Hiển thị",
    SUMMARY_UNIT: "đơn",
    SUMMARY_DIVIDER: "/",
    SHEET_TITLE: "Chi tiết đơn nghỉ",
    SHEET_END_DATE: "Ngày kết thúc",
    SHEET_TOTAL_TIME: "Tổng thời gian",
    CREATE_TITLE: "Tạo Đơn Nghỉ Phép",
    CREATE_DESC: "Vui lòng điền thông tin để xin nghỉ phép.",
    CREATE_TYPE: "Loại phép",
    CREATE_TYPE_PLACEHOLDER: "Chọn loại phép...",
    CREATE_DATE_START: "Ngày bắt đầu",
    CREATE_DATE_END: "Ngày kết thúc",
    CREATE_DATE_PLACEHOLDER: "Chọn ngày",
    CREATE_REASON: "Lý do nghỉ",
    CREATE_REASON_PLACEHOLDER: "Nhập lý do chi tiết...",
    CREATE_WARNING:
      "Đơn xin nghỉ phép sẽ được gửi đến quản lý trực tiếp của bạn để phê duyệt.",
    CREATE_BTN_CANCEL: "Hủy",
    CREATE_BTN_SUBMIT: "Gửi đơn",
    CREATE_BTN_SUBMITTING: "Đang gửi...",
    TYPE_ANNUAL: "Nghỉ phép năm",
    TYPE_SICK: "Nghỉ ốm",
    TYPE_UNPAID: "Nghỉ không lương",
    TYPE_PERSONAL: "Việc riêng",
    EMPTY_PENDING: "Không có yêu cầu nghỉ phép nào đang chờ duyệt.",
    MSG_SUBMIT_SUCCESS: "Đơn xin nghỉ phép đã được gửi thành công!",
    MSG_INSUFFICIENT_BALANCE: "Số dư phép không đủ cho yêu cầu này.",
    MSG_FETCH_EMPLOYEE_ERROR: "Không lấy được thông tin nhân viên hiện tại",
    DETAIL_INFO_GROUP: "Thông tin chung",
    DETAIL_LEAVE_GROUP: "Chi tiết nghỉ phép",
    DETAIL_REASON_GROUP: "Lý do chi tiết",
    DETAIL_TIMELINE_GROUP: "Lịch sử hoạt động",
  },
  // File Uploads
  DOCUMENTS: {
    UPLOAD_NEW: "Tải lên tài liệu mới",
    DRAG_DROP: "Kéo & thả tệp vào đây",
    OR_BROWSE: "hoặc nhấn để duyệt tệp",
    ALLOWED_FORMATS: "Định dạng cho phép: PDF, JPG, PNG",
    MAX_SIZE: (size: number) => `Kích thước tối đa: ${size}MB`,
    READY_UPLOAD: (size: number) => `Sẵn sàng tải lên (${size} MB)`,
    STATUS_SIGNED: "Đã ký",
    STATUS_VERIFIED: "Đã xác minh",
    DOC_1_NAME: "Bản sao CCCD/CMND",
    DOC_2_NAME: "Sơ yếu lý lịch (CV)",
    DOC_3_NAME: "Bằng cấp & Chứng chỉ",
  },
  // Adjustment Request
  ADJUSTMENT: {
    EMPTY_FILTER_TITLE: "Không tìm thấy kết quả phù hợp",
    EMPTY_FILTER_DESC: "Thử thay đổi bộ lọc hoặc từ khóa tìm kiếm",
    EMPTY_TITLE: "Bạn chưa có yêu cầu điều chỉnh nào",
    EMPTY_DESC: 'Nhấn "+ Tạo yêu cầu mới" để bắt đầu',
    BREADCRUMB: "Chấm công / Yêu cầu điều chỉnh",
    TITLE: "Yêu cầu điều chỉnh chấm công",
    DESC: "Theo dõi lịch sử và trạng thái các yêu cầu điều chỉnh chấm công của bạn.",
    BTN_CREATE: "Tạo yêu cầu mới",
    SEARCH_PLACEHOLDER: "Tìm kiếm mã, lý do...",
    FILTER_STATUS: "Trạng thái",
    FILTER_TYPE: "Loại chấm công",
    BTN_CLEAR: "Xóa bộ lọc",
    TABLE_ID: "Mã yêu cầu",
    TABLE_DATE_CREATED: "Ngày tạo",
    TABLE_DATE_ADJUSTMENT: "Ngày điều chỉnh",
    TABLE_TYPE: "Loại chấm công",
    TABLE_TIME_PROPOSED: "Thời gian đề xuất",
    TABLE_STATUS: "Trạng thái",
    BTN_DETAIL: "Xem chi tiết",
    BTN_EDIT_RESEND: "Chỉnh sửa & gửi lại",
    BTN_RESEND: "Gửi lại",
    SUMMARY_TOTAL: "Tổng",
    SUMMARY_UNIT: "yêu cầu",
    MSG_FETCH_ERROR: "Không thể tải danh sách yêu cầu.",
    MSG_SUBMIT_SUCCESS: "Yêu cầu đã được gửi thành công!",
    MSG_SUBMIT_DESC: "Yêu cầu đang chờ quản lý phê duyệt.",
    MSG_RESUBMIT_SUCCESS: "Đã gửi lại yêu cầu thành công!",
    SHEET_TIME_SECTION: "Thời gian điều chỉnh",
    SHEET_OLD_CHECKIN: "Check-in cũ",
    SHEET_OLD_CHECKOUT: "Check-out cũ",
    SHEET_PROPOSED: "Đề xuất",
    SHEET_REASON: "Lý do",
    SHEET_HISTORY: "Lịch sử thao tác",
    SHEET_CREATED_AT: "Tạo lúc",
    SHEET_EMPTY: "—",
    CREATE_TITLE: "Tạo Yêu cầu Điều chỉnh",
    CREATE_DESC: "Nhập thông tin để cập nhật nhật ký chấm công của bạn.",
    CREATE_DATE_LABEL: "Ngày cần điều chỉnh",
    CREATE_DATE_PLACEHOLDER: "Chọn ngày",
    CREATE_DATE_WARNING: "Không thể chọn ngày trong tương lai.",
    CREATE_TYPE_LABEL: "Loại điều chỉnh",
    CREATE_TYPE_PLACEHOLDER: "Chọn loại điều chỉnh...",
    CREATE_TIME_IN_TRUE: "Giờ Check-in đúng",
    CREATE_TIME_OUT_TRUE: "Giờ Check-out đúng",
    CREATE_TIME_ONLY: "Thời gian đúng",
    CREATE_REASON_LABEL: "Lý do chi tiết",
    CREATE_REASON_PLACEHOLDER:
      "Mô tả rõ lý do cần điều chỉnh (máy lỗi, quên quẹt thẻ...)",
    CREATE_BTN_CANCEL: "Hủy",
    CREATE_BTN_SUBMIT: "Gửi yêu cầu",
    CREATE_BTN_SUBMITTING: "Đang gửi...",
  },

  // Unified Requests (Leave + Adjustment)
  REQUEST: {
    HEADING: "Yêu cầu của tôi",
    TITLE: "Quản lý yêu cầu",
    DESC: "Tạo và quản lý các yêu cầu nghỉ phép, điều chỉnh chấm công và các yêu cầu khác.",
    BTN_CREATE: "Tạo yêu cầu",
    CREATE_LEAVE: "Xin nghỉ phép",
    CREATE_ADJUSTMENT: "Điều chỉnh chấm công",
    CREATE_OTHER: "Yêu cầu khác",
    SEARCH_PLACEHOLDER: "Tìm kiếm yêu cầu...",
    FILTER_STATUS: "Trạng thái",
    FILTER_TYPE: "Loại yêu cầu",
    FILTER_ALL_STATUS: "Tất cả trạng thái",
    FILTER_ALL_TYPE: "Tất cả loại",
    TABLE_ID: "Mã yêu cầu",
    TABLE_DATE_CREATED: "Ngày tạo",
    TABLE_TYPE: "Loại yêu cầu",
    TABLE_STATUS: "Trạng thái",
    TABLE_REASON: "Vấn đề",
    EMPTY_TITLE: "Chưa có yêu cầu nào",
    EMPTY_DESC: "Bạn chưa tạo yêu cầu nào.",
    EMPTY_FILTER_TITLE: "Không tìm thấy yêu cầu nào",
    EMPTY_FILTER_DESC: "Thử điều chỉnh bộ lọc hoặc tìm kiếm khác.",
  },

  // Asset Management
  ASSET: {
    TITLE: "Quản lý tài sản",
    SEARCH_PLACEHOLDER: "Tìm kiếm tài sản...",
    BTN_ADD: "+ Thêm tài sản",
    FILTER_ALL: "Tất cả",
    FILTER_AVAILABLE: "Sẵn dùng",
    FILTER_ASSIGNED: "Đang cấp phát",
    FILTER_RETURNED: "Đã thu hồi",
    TABLE_ID: "Mã tài sản",
    TABLE_NAME: "Tên tài sản",
    TABLE_TYPE: "Loại",
    TABLE_STATUS: "Trạng thái",
    TABLE_USER: "Người sử dụng",
    TABLE_ACTIONS: "Thao tác",
    PAGINATION_SHOW: "Hiển thị",
    PAGINATION_ON: "trên",
    PAGINATION_ITEMS: "tài sản",
    MSG_DELETE_SUCCESS: "Đã xóa tài sản thành công.",
    MSG_DELETE_ERROR: "Không thể xóa tài sản. Vui lòng thử lại.",
    MSG_CANNOT_DELETE: "Không thể xóa tài sản",
    MSG_CONFIRM_DELETE: "Xác nhận xóa tài sản",
    MSG_DELETE_WARNING: "đang được cấp phát. Phải ",
    MSG_DELETE_CONFIRM: "Bạn có chắc muốn xóa tài sản ",
    MSG_DELETE_IRREVERSIBLE: "Hành động này không thể hoàn tác.",
    MSG_HOW_TO: "Cách thực hiện:",
    MSG_STEP_1: "Mở chi tiết tài sản → bấm ",
    MSG_STEP_2: "Thu hồi",
    MSG_STEP_3: " → quay lại và xóa.",
    BTN_OPEN_DETAIL: "Mở chi tiết để thu hồi",
    BTN_CLOSE: "Đóng",
    BTN_DELETE: "Xóa tài sản",
    LOADING_DELETE: "Đang xóa...",
    TITLE_DOWNLOAD_CSV: "Tải về danh sách tài sản (CSV)",
    PREFIX_ASSET: "Tài sản",
  },

  // Internal Announcement Management
  ANNOUNCEMENT: {
    TITLE: "Thông báo Nội bộ",
    TITLE_MGMT: "Quản lý Thông báo Nội bộ",
    DESC_MGMT: "Xem và xử lý các thông báo, sự kiện và chính sách của công ty.",
    LIST_TITLE: "Danh sách thông báo",
    BTN_CREATE: "Tạo thông báo mới",
    FORM_TITLE: "Tạo thông báo nội bộ",
    LABEL_TITLE: "Tiêu đề",
    LABEL_CONTENT: "Nội dung",
    LABEL_TYPE: "Phân loại",
    LABEL_TARGET: "Đối tượng nhận",
    LABEL_SEND_EMAIL: "Tạo & Gửi Gmail",
    TYPE_POLICY: "Chính sách",
    TYPE_EVENT: "Sự kiện",
    TYPE_OTHER: "Khác",
    EMPTY_LIST: "Không có thông báo nào.",
    LOADING: "Đang tải thông báo...",
    MSG_CREATE_SUCCESS: "Tạo thông báo thành công!",
    MSG_CREATE_ERROR: "Không thể tạo thông báo",
    MSG_FETCH_ERROR: "Không thể tải danh sách thông báo.",
    STATUS_READ: "Đã đọc",
    STATUS_UNREAD: "Chưa đọc",
    MARK_READ: "Đánh dấu đã đọc",
    TYPE_LABEL: {
      POLICY: "Chính sách",
      EVENT: "Sự kiện",
      OTHER: "Khác",
    },
    LOADING_NOTIFICATIONS: "Đang tải thông báo...",
    NO_NOTIFICATIONS: "Không có thông báo nào.",
    NOTIFICATION_BADGE: "Mới",
    UPDATE_READ_STATUS_ERROR: "Không thể cập nhật trạng thái đã đọc",
    NOTIFICATION_PREFIX: "Thông báo: ",
    HEADER_TITLE: "Thông báo",
    NO_NOTIFICATIONS_USER: "Bạn chưa có thông báo nào.",
    FORM_TITLE_REQUIRED: "Tiêu đề là bắt buộc",
    FORM_TITLE_MAX: "Tiêu đề tối đa 255 ký tự",
    FORM_CONTENT_REQUIRED: "Nội dung là bắt buộc",
    FORM_SELECT_AUDIENCE: "Chọn đối tượng",
    FORM_AUDIENCE_ALL: "Toàn công ty",
    FORM_AUDIENCE_DEPARTMENT: "Theo phòng ban",
    FORM_AUDIENCE_ROLE: "Theo vai trò",
    FORM_SELECT_AT_LEAST_ONE: "Vui lòng chọn ít nhất một đối tượng",
    FORM_PLACEHOLDER_TITLE: "Nhập tiêu đề thông báo",
    FORM_PLACEHOLDER_CONTENT: "Nhập nội dung thông báo",
    NO_DATA_MATCHES: "Không có dữ liệu phù hợp",
    EMAIL_SENT_COUNT: (count: number) => `, đã gửi email ${count} người`,
    SUCCESS_RECIPIENTS: (count: number, emailText: string) =>
      `Tạo thông báo thành công (${count} người nhận${emailText})`,
  },

  // Employee Management (Admin Dashboard + HR)
  EMPLOYEE: {
    TITLE: "Danh sách nhân viên",
    DESC_MGMT: "Quản lý thông tin hồ sơ và trạng thái làm việc của nhân viên",
    BTN_ADD: "Thêm nhân viên",
    BTN_CLEAR_FILTER: "Xóa lọc",
    LABEL_TOTAL: "Tổng số:",
    LABEL_UNIT: "nhân viên",
    MODAL_CREATE_TITLE: "Thêm nhân viên mới",
    MODAL_UPDATE_TITLE: "Cập nhật hồ sơ nhân viên",
    SECTION_PERSONAL: "Thông tin cá nhân",
    SECTION_JOB: "Thông tin công việc",
    SECTION_BASIC: "Thông tin cơ bản",
    SECTION_FINANCE_NOTES: "Tài chính & Ghi chú",
    SECTION_CURRENT_JOB: "Công việc hiện tại",
    LABEL_AVATAR: "Ảnh đại diện",
    LABEL_STATUS: "Trạng thái",
    LABEL_EMP_CODE: "Mã nhân viên",
    LABEL_NAME: "Họ và tên",
    LABEL_NATIONAL_ID: "CMND/CCCD",
    LABEL_DOB: "Ngày sinh",
    LABEL_GENDER: "Giới tính",
    LABEL_PHONE: "Số điện thoại",
    LABEL_EMAIL: "Email liên hệ",
    LABEL_COMP_EMAIL: "Email công ty",
    LABEL_FIRST_NAME: "Tên",
    LABEL_LAST_NAME: "Họ",
    LABEL_ADDRESS: "Địa chỉ thường trú",
    LABEL_DEPARTMENT: "Phòng ban",
    LABEL_POSITION: "Vị trí công việc",
    LABEL_MANAGER: "Quản lý trực tiếp",
    LABEL_JOIN_DATE: "Ngày vào làm",
    LABEL_END_DATE: "Ngày kết thúc (tùy chọn)",
    LABEL_CONTRACT: "Loại hợp đồng",
    LABEL_SALARY: "Lương cơ bản",
    LABEL_NOTES: "Ghi chú",
    PLACEHOLDER_NOTES: "Nhập ghi chú thêm...",
    PLACEHOLDER_LAST_NAME: "Nhập họ...",
    PLACEHOLDER_FIRST_NAME: "Nhập tên...",
    PLACEHOLDER_EMAIL: "example@company.com",
    PLACEHOLDER_PHONE: "Nhập số điện thoại...",
    PLACEHOLDER_NATIONAL_ID: "Số CCCD/CMND...",
    PLACEHOLDER_ADDRESS: "Số nhà, tên đường...",
    PLACEHOLDER_CITY: "Tỉnh/Thành phố...",
    PLACEHOLDER_EMERGENCY_NAME: "Họ tên người liên hệ...",
    PLACEHOLDER_EMERGENCY_PHONE: "Số điện thoại khẩn cấp...",
    PLACEHOLDER_BANK_NAME: "Tên ngân hàng (VD: Vietcombank)...",
    LABEL_CITY: "Tỉnh/Thành phố",
    LABEL_EMERGENCY_PHONE: "Số điện thoại",
    LABEL_BANK_NAME: "Tên ngân hàng",
    HINT_SELECT_DEPT: "Vui lòng chọn phòng ban trước",
    HINT_CHOOSE_POSITION: "Vui lòng chọn vị trí trước",
    MSG_NO_MANAGERS: "Không tìm thấy quản lý nào phù hợp cho vị trí này.",
    LABEL_AVATAR_DESC:
      "Tải lên hoặc kéo thả ảnh chân dung chuyên nghiệp (JPG, PNG).",
    BTN_UPLOAD: "Chọn tệp",
    SELECT_DEPT: "Chọn phòng ban...",
    SELECT_POSITION: "Chọn vị trí...",
    LABEL_REPORTING_MANAGER: "Trưởng phòng / Quản lý",
    OPTION_NO_MANAGER: "— Không chỉ định —",
    MSG_MANAGER_LEVEL: "Cấp bậc Trưởng phòng / Quản lý.",
    LABEL_EMP_CODE_SHORT: "Mã NV",
    CONTRACT_TYPES: {
      FULL_TIME: "Toàn thời gian (Full-time)",
      PART_TIME: "Bán thời gian (Part-time)",
      CONTRACT: "Hợp đồng có thời hạn",
      INTERN: "Thực tập sinh",
      CONSULTANT: "Tư vấn viên",
      TEMPORARY: "Thời vụ",
    },
    GENDER_MALE: "Nam",
    GENDER_FEMALE: "Nữ",
    GENDER_OTHER: "Khác",
    STATUS_ACTIVE: "Đang làm việc",
    STATUS_INACTIVE: "Nghỉ việc",
    STATUS_PENDING: "Chưa xác định",
    CONTRACT_INDEFINITE: "Hợp đồng không xác định thời hạn",
    CONTRACT_ONE_YEAR: "Hợp đồng 1 năm",
    CONTRACT_PROBATION: "Thử việc",
    BTN_CANCEL: "Hủy",
    BTN_UPDATE: "Cập nhật",
    BTN_CREATE: "Tạo mới",
    BTN_SAVE_PROFILE: "Lưu hồ sơ",
    MSG_UPDATE_SUCCESS: "Cập nhật hồ sơ thành công!",
    MSG_CREATE_SUCCESS: "Tạo nhân viên thành công!",
    MSG_DELETE_SUCCESS: "Đã xóa nhân viên.",
    MSG_CREATE_ERROR: "Không thể tạo nhân viên.",
    MSG_UPDATE_ERROR: "Không thể cập nhật nhân viên.",
    MSG_DELETE_ERROR: "Không thể xóa nhân viên.",
    MSG_VALIDATION_ERROR: "Vui lòng điền đầy đủ nội dung",
    MSG_DELETE_CONFIRM: "Bạn có chắc chắn muốn xóa nhân viên này?",
    MSG_FETCH_ERROR: "Không thể tải danh sách nhân viên.",
    INFO_NOTE:
      "Các thay đổi về chức danh và phòng ban sẽ cần sự phê duyệt của trưởng bộ phận.",
    DEPT_SOFTWARE: "Phòng Phát triển phần mềm",
    DEPT_HR: "Phòng Nhân sự",
    DEPT_SALES: "Phòng Kinh doanh",
    DEPT_BOD: "Ban Giám đốc",
    MGR_A: "An Nguyễn",
    MGR_B: "Bình Trần",
    MGR_C: "Chi Lê",
    MODAL_DETAIL_TITLE: "Chi tiết nhân viên",
    LOADING_DETAIL: "Đang tải thông tin nhân viên...",
    LABEL_NATIONALITY: "Quốc tịch",
    LABEL_PROBATION_END: "Kết thúc thử việc",
    LABEL_LEAVE_ANNUAL: "Phép năm tồn",
    LABEL_LEAVE_SICK: "Phép bệnh tồn",
    SECTION_EMERGENCY: "Liên hệ khẩn cấp",
    LABEL_EMERGENCY_NAME: "Họ tên",
    LABEL_EMERGENCY_RELATION: "Quan hệ",
    SECTION_FINANCE: "Tài chính & Thuế",
    LABEL_TAX_ID: "Mã số thuế",
    LABEL_SOCIAL_ID: "Số sổ BHXH",
    LABEL_BANK_ACCOUNT: "Số tài khoản",
    LABEL_BANK_BRANCH: "Chi nhánh",
    STATUS_ACTIVE_CAP: "ĐANG LÀM VIỆC",
    STATUS_INACTIVE_CAP: "NGHỈ VIỆC",
    DEFAULT_WORK_LOCATION: "Văn phòng chính",
    DEFAULT_NATIONALITY: "Việt Nam",
    INFO_EMP_CODE: "Mã nhân viên sẽ được hệ thống tự động tạo.",
    SYSTEM_HR: "Hệ thống Nhân sự",
    PLACEHOLDER_SOCIAL_WARRANTY_NUMBER: "Số bảo hiểm xã hội...",
    LABEL_SOCIAL_WARRANTY_NUMBER: "Số bảo hiểm xã hội",
    SECTION_ATTACHMENTS: "Tài liệu đính kèm",
    TXT_CLICK_OR_DRAG_TO_UPLOAD: "Click hoặc kéo thả file vào đây để tải lên",
    TXT_SUPPORTED_FILE_TYPES: "Hỗ trợ PDF, DOCX, JPG, PNG (Tối đa 10MB)",
    MSG_DELETE_DESC:
      "Hành động này có thể bị chặn nếu nhân viên có dữ liệu ràng buộc.",
    TXT_REMOVE: "Xóa",
    LABEL_CONTRACT_TERM: "Kỳ hạn hợp đồng có thời hạn",
    LABEL_MONTHS: (count: number) => `${count} tháng`,
    LABEL_CONTRACT_START: "Ngày bắt đầu hợp đồng",
    LABEL_AVATAR_PREVIEW: "Ảnh đại diện xem trước",
    PLACEHOLDER_AVATAR_URL: "Nhập URL ảnh đại diện (Tùy chọn)",
    LABEL_SELECT_AVATAR: "Chọn ảnh đại diện",
    STATUS_PROBATION: "Thử việc",
    BTN_CONFIRM_OFFICIAL: "Xác nhận chính thức",
    MSG_CONVERT_SUCCESS: "Xác nhận chính thức thành công",
    MSG_CONVERT_ERROR: "Không thể xác nhận chính thức. Vui lòng thử lại.",
    MODAL_CONVERT_TITLE: "Xác nhận chính thức",
    LABEL_SIGNING_DATE: "Ngày ký HĐ chính thức",
    LABEL_OFFICIAL_SALARY_VND: "Mức lương chính thức mới (VNĐ)",
    CONTRACT_TERM_AUTO:
      "Hệ thống sẽ tự tính ngày hết hạn hợp đồng theo kỳ hạn đã chọn.",
    TERM_ONE_YEAR: "1 năm",
    TERM_TWO_YEARS: "2 năm",
    TERM_THREE_YEARS: "3 năm",
    TERM_INDEFINITE: "Không thời hạn",
    CONTRACT_TYPE_OFFICIAL: "Loại hợp đồng chính thức",
    PLACEHOLDER_SIGNING_DATE: "Vui lòng chọn ngày ký hợp đồng",
    PLACEHOLDER_OFFICIAL_SALARY: "Vui lòng nhập mức lương chính thức",
    MSG_SALARY_MIN: "Mức lương phải lớn hơn 0",
  },

  // Common Toast Messages
  TOAST: {
    LEAVE_SUBMITTED: "Đơn nghỉ phép đã được gửi!",
    LEAVE_SUBMITTED_DESC: (id: string) =>
      `Mã ${id} đang chờ quản lý phê duyệt.`,
    LEAVE_CANCELLED: "Đã hủy đơn nghỉ phép.",
  },

  ATTENDANCE: {
    TITLE: "Lịch sử điểm danh",
    FILTER_STATUS: "Lọc trạng thái",
    SEARCH_PLACEHOLDER: "Tìm kiếm...",
    TABLE_DATE: "Ngày",
    TABLE_CHECKIN: "Check-in",
    TABLE_CHECKOUT: "Check-out",
    TABLE_WORK_HOURS: "Giờ làm",
    TABLE_STATUS: "Trạng thái",
    TABLE_METHOD: "Phương thức",
    STATS_PRESENT: "Ngày có mặt",
    STATS_LATE: "Ngày muộn",
    STATS_ABSENT: "Ngày vắng",
    STATS_WORK_HOURS: "Tổng giờ",
    METHOD_CAMERA_GPS: "Camera+GPS",
    METHOD_MANUAL: "Thủ công",
    NO_DATA: "Chưa có dữ liệu",
  },

  // Attendance Settings
  ATTENDANCE_SETTINGS: {
    MSG_GEOLOCATION_NOT_SUPPORTED: "Trình duyệt không hỗ trợ định vị.",
    MSG_CANNOT_GET_LOCATION: "Không thể lấy vị trí hiện tại.",
  },

  // Approve Leave/Adjustment (Manager)
  APPROVE: {
    LEAVE_TITLE: "Phê duyệt đơn nghỉ",
    ADJUSTMENT_TITLE: "Phê duyệt điều chỉnh",
    SEARCH_PLACEHOLDER: "Tìm kiếm nhân viên...",
    SEARCH_EMP_PLACEHOLDER: "Tìm nhân viên hoặc mã...",
    LOADING: "Đang tải dữ liệu...",
    FEEDBACK_PLACEHOLDER: "Nhập nội dung phản hồi cho nhân viên...",
    BTN_APPROVE: "Phê duyệt",
    BTN_REJECT: "Từ chối",
    BTN_REQUEST_MORE: "Yêu cầu bổ sung",
    STATUS_ALL: "Tất cả",
    STATS_TOTAL: "Tổng",
    STATS_PENDING: "Chờ duyệt",
    STATS_APPROVED: "Đã duyệt",
    STATS_REJECTED: "Đã từ chối",
    TOAST_APPROVED: "Đã phê duyệt đơn nghỉ phép",
    TOAST_REJECTED: "Đã từ chối đơn nghỉ phép",
    TOAST_RETURNED: "Đã trả đơn về cho nhân viên",
    TOAST_ADJUSTMENT_APPROVED: "Đã duyệt yêu cầu thành công!",
    TOAST_ADJUSTMENT_REJECTED: "Đã từ chối yêu cầu.",
    TOAST_ADJUSTMENT_RETURNED: "Đã trả yêu cầu về nhân viên.",
    DURATION_DAYS: (days: number) => `${days} ngày`,
    STATUS_APPROVED: "Đã duyệt",
    STATUS_REJECTED: "Từ chối",
    LEAVE_LIST_DESC: "Xử lý các yêu cầu nghỉ phép từ nhân viên.",
    BTN_EXPORT: "Xuất báo cáo",
    SEARCH_EMP: "Tìm kiếm nhân viên...",
    FILTER_ALL: "Tất cả",
    TABLE_COL_EMP: "Nhân viên",
    TABLE_COL_DEPT: "Phòng ban",
    TABLE_COL_TYPE: "Loại nghỉ",
    TABLE_COL_TIME: "Thời gian",
    TABLE_COL_STATUS: "Trạng thái",
    LOADING_DATA: "Đang tải dữ liệu...",
    VIEW_DETAIL: "Xem chi tiết",
    UNIT_DAYS: "n",
    DISPLAY_PREFIX: "Hiển thị",
    DISPLAY_UNIT: "yêu cầu",
    SECTION_GENERAL: "THÔNG TIN CHUNG",
    LABEL_EMP_CODE_SHORT: "Mã NV",
    SECTION_LEAVE_DETAIL: "CHI TIẾT NGHỈ PHÉP",
    SECTION_REASON_DETAIL: "LÝ DO CHI TIẾT",
    LABEL_FEEDBACK: "Ý kiến phản hồi",
    PLACEHOLDER_ID: "EMP-001",
    PLACEHOLDER_TIME: "08:30, 01/03/2026",
  },

  // Asset Detail Modal
  ASSET_DETAIL: {
    TITLE: "Chi tiết tài sản",
    SECTION_BASIC: "Thông tin cơ bản",
    SECTION_SUPPORT: "Thông tin phụ trợ",
    SECTION_HISTORY: "Lịch sử tài sản",
    LABEL_CODE: "Mã tài sản",
    LABEL_NAME: "Tên tài sản",
    LABEL_TYPE: "Loại tài sản",
    LABEL_VALUE: "Giá trị",
    LABEL_PURCHASE_DATE: "Ngày mua",
    LABEL_STATUS: "Trạng thái",
    LABEL_CONDITION: "Tình trạng",
    LABEL_WARRANTY: "Bảo hành",
    LABEL_SUPPLIER: "Nhà cung cấp",
    LABEL_CONTRACT: "Hợp đồng",
    LABEL_LOCATION: "Vị trí",
    LABEL_USER: "Người sử dụng",
    LABEL_ASSIGNED_DATE: "Ngày cấp phát",
    LABEL_LOCATION_USER: "Người sử dụng",
    LABEL_DESCRIPTION: "Mô tả",
    LABEL_DESC_DETAIL: "Mô tả chi tiết",
    MSG_FETCH_ERROR: "Không thể tải thông tin tài sản.",
    MSG_ASSIGN_SUCCESS: "Cấp phát tài sản thành công!",
    MSG_ASSIGN_ERROR: "Không thể cấp phát tài sản. Vui lòng thử lại.",
    MSG_RETURN_SUCCESS: "Thu hồi tài sản thành công!",
    MSG_RETURN_ERROR: "Không thể thu hồi tài sản. Vui lòng thử lại.",
    BTN_ASSIGN: "Cấp phát",
    BTN_RETURN: "Thu hồi",
    BTN_VIEW_HISTORY: "Xem lịch sử đầy đủ",
    BTN_CONFIRM_ASSIGN: "Xác nhận cấp phát",
    BTN_CONFIRM_RETURN: "Xác nhận thu hồi",
    PLACEHOLDER_EMP: "Tìm nhân viên theo tên hoặc mã...",
    PLACEHOLDER_NOTE: "Nhập ghi chú (nếu có)...",
    LABEL_ASSIGN_NOTE: "Ghi chú cấp phát",
    LABEL_RETURN_NOTE: "Ghi chú thu hồi",
    LABEL_RETURN_FROM: "Thu hồi từ",
    MSG_NOT_FOUND: "Không tìm thấy thông tin tài sản",
    MSG_SEARCHING: "Đang tìm kiếm...",
    MSG_NO_EMPLOYEES: "Không tìm thấy tài khoản nhân viên.",
    TXT_PACKAGE_ICON: "📦",
    TXT_COLON_NOSPACE: ":",
    TXT_DASH: "—",
    TXT_NO_HISTORY: "Chưa có lịch sử hoạt động.",
    TXT_CONDITION_RETURN: "Tình trạng khi thu hồi",
    TXT_READY_REUSE: "Sẵn sàng tái sử dụng ngay",
    TYPE_TECH: "Thiết bị công nghệ",
    STATUS_NEW: "Mới",
    STATUS_NORMAL: "Bình thường",
    STATUS_MAINTENANCE: "Cần bảo trì",
  },

  // Asset History
  ASSET_HISTORY: {
    TITLE: "Lịch sử biến động tài sản",
    DESC: "Theo dõi toàn bộ quá trình luân chuyển và thay đổi của tài sản.",
    LOADING: "Đang tải lịch sử tài sản...",
    EMPTY_TITLE: "Chưa có lịch sử",
    EMPTY_DESC: "Tài sản này chưa có ghi nhận biến động nào.",
    MSG_FETCH_ERROR: "Không thể tải lịch sử tài sản.",
    ACTION_CREATE: "Tạo mới",
    ACTION_ASSIGN: "Cấp phát",
    ACTION_RETURN: "Thu hồi",
    DONE_BY: "Thực hiện bởi",
  },

  // Asset Create/Edit Modal
  ASSET_CREATE: {
    TITLE_ADD: "Thêm tài sản mới",
    TITLE_EDIT: "Chỉnh sửa tài sản",
    SECTION_SUPPLIER: "Thông tin nhà cung cấp",
    SECTION_INFO: "Thông tin tài sản",
    PLACEHOLDER_SUPPLIER: "Nhập tên nhà cung cấp...",
    PLACEHOLDER_NAME: "Nhập tên tài sản...",
    PLACEHOLDER_CODE: "Nhập mã tài sản...",
    PLACEHOLDER_VALUE: "0",
    PLACEHOLDER_LOCATION: "VD: Nguyễn Văn A...",
    PLACEHOLDER_USER_SEARCH: "Tìm và chọn nhân viên theo tên...",
    PLACEHOLDER_DESC: "Mô tả thêm về cấu hình, tình trạng...",
    LABEL_SUPPLIER: "Nhà cung cấp",
    LABEL_NAME: "Tên tài sản",
    LABEL_CODE: "Mã tài sản",
    LABEL_TYPE: "Loại tài sản",
    LABEL_VALUE: "Giá trị (VNĐ)",
    LABEL_PURCHASE_DATE: "Ngày mua",
    LABEL_STATUS: "Trạng thái",
    LABEL_CONDITION: "Tình trạng",
    LABEL_WARRANTY: "Bảo hành đến",
    LABEL_LOCATION: "Vị trí/ Người sử dụng",
    LABEL_DESC: "Mô tả",
    // New keys added to fix errors
    MSG_REQUIRE_NAME: "Vui lòng nhập tên tài sản",
    MSG_REQUIRE_TYPE: "Vui lòng nhập loại tài sản",
    MSG_REQUIRE_LOCATION: "Vui lòng chọn người sử dụng",
    MSG_CREATE_SUCCESS: "Tạo tài sản thành công!",
    MSG_CREATE_ERROR: "Không thể tạo tài sản. Vui lòng thử lại.",
    MSG_ERROR_VALUE: "Giá trị không được âm",
    MSG_ERROR_PURCHASE_DATE: "Ngày mua không được ở tương lai",
    LABEL_IMAGE: "Hình ảnh tài sản",
    UPLOAD_IMAGE: "Nhấn hoặc kéo thả để tải ảnh",
    PLACEHOLDER_IMAGE_URL: "Hoặc nhập URL hình ảnh...",
    SECTION_ADDITIONAL: "Thông tin bổ sung",
    LABEL_CONTRACT_NUM: "Số hợp đồng",
    PLACEHOLDER_CONTRACT_NUM: "Nhập số hợp đồng...",
    LABEL_CONTRACT_UNTIL: "Hợp đồng đến ngày",
    LABEL_AUTO_CODE: "Mã sẽ được hệ thống tự động khởi tạo",
    LABEL_INITIAL_STATUS: "Trạng thái ban đầu",
    LABEL_LOCATION_ONLY: "Người sử dụng",
    LABEL_USER_ONLY: "Người sử dụng",
    LABEL_CONDITION_ONLY: "Tình trạng",
    LABEL_NOTES: "Ghi chú/Mô tả chi tiết",
    TXT_CODE_BRACKET_START: " [",
    TXT_CODE_BRACKET_END: "]",
    TXT_REQUIRED_MARK: "*",
    LABEL_CONTRACT_DATE: "Ngày hợp đồng",
  },

  // Asset Group Management
  ASSET_GROUP: {
    TITLE: "Tài sản nhóm quản lý",
    TITLE_DESC: "Theo dõi thiết bị đang được cấp phát trong nhóm của bạn",
    STATS_TOTAL: "Tổng tài sản",
    STATS_UNIT: "tài sản trong nhóm",
    STATS_AVAILABLE: "Sẵn dùng",
    STATS_IN_USE: "Đang sử dụng",
    STATS_MAINTENANCE: "Bảo trì",
    FILTER_STATUS_ALL: "Tất cả trạng thái",
    FILTER_TYPE: "Loại tài sản",
    FILTER_ALL: "Tất cả",
    TABLE_ASSIGNED_TO: "Đang gán cho",
    PAGE_PREFIX: "Trang ",
    FETCH_ERROR: "Không thể tải tài sản nhóm",
  },

  // Asset Report (Admin)
  ASSET_REPORT: {
    MSG_FETCH_LIST_ERROR: "Không thể tải danh sách báo cáo.",
    MSG_FETCH_DETAIL_ERROR: "Không thể tải chi tiết báo cáo.",
    MSG_APPROVE_SUCCESS: "Đã phê duyệt báo cáo thành công!",
    MSG_APPROVE_DESC: "Hệ thống đã cập nhật trạng thái tài sản theo báo cáo.",
    MSG_REJECT_SUCCESS: "Đã từ chối báo cáo thành công!",
    MSG_PROCESS_ERROR: "Lỗi xử lý báo cáo.",
    MSG_TRY_AGAIN: "Vui lòng thử lại sau.",
    MSG_SELF_REVIEW_ERROR: "Bạn không thể tự duyệt báo cáo sự cố của bản thân.",

    SUBTITLE: "Báo cáo sự cố",
    TITLE: "Quản lý Báo cáo Tài sản",
    DESC: "Xem và xử lý các báo cáo sự cố, hư hỏng hoặc mất mát tài sản từ nhân viên.",
    SEARCH_PLACEHOLDER: "Tìm kiếm mã báo cáo, mã tài sản...",

    STATS: {
      TOTAL: "Tổng số",
      PENDING: "Chờ duyệt",
      APPROVED: "Đã duyệt",
      REJECTED: "Đã từ chối",
    },

    INCOMING_REPORTS: "Danh sách báo cáo lưu chuyển",

    TABLE_COLS: {
      CODE: "Mã",
      EMPLOYEE: "Nhân viên",
      ASSET: "Tài sản",
      ISSUE_TYPE: "Loại sự cố",
      REPORT_DATE: "Ngày báo cáo",
      STATUS: "Trạng thái",
      ACTIONS: "Thao tác",
    },

    TXT_HASH: "#",
    TXT_QUOTE: '"',
    TXT_DAMAGED: "Hư hỏng",
    TXT_LOST: "Mất mát",
    TXT_NO_ATTACHMENT: "Không có tệp đính kèm",
    TXT_NO_NOTE: "Không có ghi chú",

    BTN_VIEW_DETAIL: "Xem chi tiết",
    BTN_REJECT: "Từ chối",
    BTN_APPROVE: "Phê duyệt",
    BTN_CLOSE_DETAIL: "Đóng",

    EMPTY_TITLE: "Chưa có báo cáo nào",
    EMPTY_DESC: "Không tìm thấy báo cáo sự cố tài sản nào trong hệ thống.",

    DETAIL_TITLE: "Chi tiết Báo cáo Sự cố",

    SECTION_ASSET_INFO: "Thông tin tài sản",
    LABEL_ASSET_NAME: "Tên tài sản",
    LABEL_ASSET_STATUS: "Trạng thái",
    LABEL_ASSET_CODE: "Mã tài sản",
    LABEL_DEVICE_CODE: "Mã thiết bị",
    LABEL_ASSET_TAG_SHORT: "Tag tài sản",
    LABEL_CURRENT_CONDITION: "Tình trạng hiện tại",
    LABEL_TARGET_UPDATE: "Sẽ cập nhật thành",
    LABEL_UPDATED_TO: "Đã cập nhật thành",

    SECTION_REPORT_CONTENT: "Nội dung báo cáo",
    LABEL_ISSUE_TYPE: "Loại sự cố",
    LABEL_DETAIL_DESC: "Mô tả chi tiết",

    SECTION_EVIDENCE: "Bằng chứng đính kèm",

    SECTION_PROCESS_INFO: "Thông tin xử lý",
    LABEL_PROCESSOR: "Người xử lý",
    LABEL_PROCESS_TIME: "Thời gian",
    LABEL_PROCESS_NOTE: "Ghi chú xử lý",

    LABEL_FEEDBACK_NOTE: "Ghi chú phản hồi",
    PLACEHOLDER_FEEDBACK: "Nhập phản hồi hoặc lý do xử lý cho nhân viên...",
    UNIT_REPORT: "báo cáo",
    PLACEHOLDER_STATUS: "Trạng thái",
  },

  // Audit Logs
  AUDIT_LOGS: {
    TITLE: "Nhật ký kiểm toán",
    DESC: "Theo dõi ai đã thao tác gì, vào thời điểm nào và từ đâu.",
    PLACEHOLDER_ACTION_TYPE: "Loại hành động",
    OPTION_ALL_ACTIONS: "Tất cả hành động",
    TABLE_ACTION: "Hành động",
    BTN_DETAIL: "Chi tiết",
    LABEL_ID: "ID",
    LABEL_TIME: "Thời gian",
    LABEL_ACTOR: "Actor",
    LABEL_ACTION: "Action",
    LABEL_IP: "IP Address",
    LABEL_CLIENT_TYPE: "Client Type",
    LABEL_IDENTIFIER: "Identifier",
    LABEL_CORRELATION_ID: "Correlation ID",
    LABEL_USER_AGENT: "User Agent",
    LABEL_NEW_VALUE: "New Value",
    BTN_FILTER: "Lọc",
    TOTAL_REC: "Tổng",
    REC_UNIT: "bản ghi",
    PAGE_PREFIX: "Trang",
    BTN_PREV: "Trước",
    BTN_NEXT: "Sau",
    MODAL_DETAIL_TITLE: "Chi tiết audit log",
    ACTION_OPTIONS: {
      LOGIN_SUCCESS: "Đăng nhập thành công",
      LOGIN_FAILED: "Đăng nhập thất bại",
      PASSWORD_CHANGED: "Đổi mật khẩu",
      TOKEN_REFRESH_SUCCESS: "Làm mới token thành công",
      TOKEN_REFRESH_FAILED: "Làm mới token thất bại",
      TOKEN_EXPIRED: "Token hết hạn",
      TOKEN_INVALID: "Token không hợp lệ",
      TOKEN_REVOKED: "Thu hồi token",
      LOGOUT: "Đăng xuất",
      ACCESS_DENIED: "Từ chối truy cập",
      ASSET_REPORT_SUBMITTED: "Gửi báo cáo sự cố",
      ASSET_REPORT_APPROVED: "Duyệt báo cáo sự cố",
      ASSET_REPORT_REJECTED: "Từ chối báo cáo sự cố",
    },
  },

  // Asset Incident Management
  ASSET_INCIDENT: {
    TITLE: "Quản lý sự cố tài sản",
    SUBTITLE:
      "Xem xét và quản lý các báo cáo mất mát và hư hỏng tài sản của nhân viên.",
    SEARCH_PLACEHOLDER: "Tìm kiếm theo tên nhân viên hoặc tài sản...",
    LABEL_STATUS: "Trạng thái:",
    TABLE_EMPLOYEE: "NHÂN VIÊN",
    TABLE_ASSET: "TÀI SẢN",
    TABLE_INCIDENT: "LOẠI SỰ CỐ",
    TABLE_DATE: "NGÀY BÁO CÁO",
    TABLE_STATUS: "TRẠNG THÁI",
    TABLE_ACTION: "THAO TÁC",
    LABEL_ID: "Mã:",
    PAGINATION_SHOW: "Đang hiển thị",
    PAGINATION_TO: "đến",
    PAGINATION_OF: "của",
    PAGINATION_RESULTS: "kết quả",
    BTN_PREV: "Trước",
    BTN_NEXT: "Tiếp",
    LOADING_DATA: "Đang tải dữ liệu...",
    NO_DATA: "Không có sự cố nào.",
  },

  ASSET_INCIDENT_MODAL: {
    TITLE: "Xem xét báo cáo sự cố",
    LABEL_ASSET_NAME: "Tên tài sản",
    LABEL_ASSET_ID: "Mã tài sản",
    LABEL_INCIDENT_TYPE: "Loại sự cố",
    LABEL_DATE: "Ngày báo cáo",
    LABEL_DESC: "Mô tả sự cố",
    LABEL_UPDATE_COND: "Cập nhật tình trạng tài sản (Bắt buộc khi Duyệt)",
    PLACEHOLDER_COND: "Chọn tình trạng mới...",
    COND_GOOD: "Tốt (Good)",
    COND_FAIR: "Trung bình (Fair)",
    COND_NEEDS_REPAIR: "Cần sửa chữa (Needs Repair)",
    BTN_APPROVE: "Duyệt",
    BTN_REJECT: "Từ chối",
    MSG_SELF_RESOLVE_ERROR:
      "Bạn không thể tự duyệt báo cáo sự cố của bản thân.",
  },

  // My Assets (Employee)
  MY_ASSETS: {
    TITLE: "Tài sản của tôi",
    DESCRIPTION:
      "Vui lòng quản lý và báo cáo sự cố với thiết bị được cấp phát của bạn.",
    BREADCRUMB: "Tài sản",
    SECTION_EQUIPMENT: "Thiết bị được cấp phát",
    SECTION_REPORTS: "Báo cáo gần đây",
    TABLE_ID: "Mã báo cáo",
    TABLE_INCIDENT: "Loại lỗi",
    TABLE_DATE: "Ngày báo",
    PLACEHOLDER_INCIDENT: "Chọn loại sự cố...",
    PLACEHOLDER_DESC: "Vui lòng mô tả chi tiết vấn đề...",
    REPORT_DESC:
      "Vui lòng cung cấp chi tiết về sự cố tài sản để giúp chúng tôi xử lý nhanh chóng.",
    LABEL_INCIDENT: "Loại sự cố",
    LABEL_DESC: "Mô tả chi tiết",
    LABEL_REQUIRED: "Bắt buộc",
    LABEL_ATTACHMENT: "Đính kèm bằng chứng",
    NO_FILE: "Chưa có tệp nào",
    FILE_LIMIT: "Tối đa 5MB (JPG, PNG, PDF)",
    BTN_CHOOSE_FILE: "Chọn tệp",
    BTN_REPORT: "Báo cáo sự cố",
    REPORT_TITLE: "Báo cáo sự cố tài sản",
    TOAST_SELECT_TYPE: "Vui lòng chọn loại sự cố",
    TOAST_DESC_MIN: "Vui lòng nhập mô tả chi tiết (tối thiểu 10 ký tự)",
    TOAST_SUCCESS: "Báo cáo thành công",
    TOAST_SUCCESS_DESC: "Báo cáo của bạn đã được gửi tới HR để xử lý.",
    TOAST_FAILED: "Gửi báo cáo thất bại",
    TOAST_ERROR_DEFAULT: "Có lỗi xảy ra, vui lòng thử lại sau.",
    MAX_FILE_SIZE_ERROR: "File quá lớn. Vui lòng chọn file dưới 5MB.",
    UNIT_MB: "MB",
    LABEL_ASSET_TAG: "Asset Tag",
    LABEL_REPORT_COUNT: "Tổng số báo cáo: ",
    STATUS_DAMAGED: "Hư hỏng / Lỗi thiết bị",
    STATUS_LOST: "Mất mát / Thất lạc",
    ALL_STATUS: "Thiết bị & Báo cáo",
    REQUEST_STATUS: "Yêu cầu cấp phát",
    WAITING_ASSIGN: "Chờ bàn giao",
    WAITING_DESC:
      "Tài sản chưa được bàn giao thực tế. Vui lòng liên hệ HR/Admin.",
    LABEL_REQUEST_ID: "Mã YC: ",
  },

  // Asset Request (Employee & Admin)
  ASSET_REQUEST: {
    TITLE_EMPLOYEE: "Yêu cầu tài sản của tôi",
    DESC_EMPLOYEE:
      "Gửi yêu cầu cấp phát tài sản mới và theo dõi trạng thái phê duyệt.",
    TITLE_ADMIN: "Duyệt yêu cầu tài sản",
    DESC_ADMIN: "Xem và phê duyệt các yêu cầu cấp phát tài sản của nhân viên.",
    BTN_CREATE: "Tạo yêu cầu mới",
    SEARCH_PLACEHOLDER: "Tìm mã yêu cầu hoặc loại tài sản...",
    SEARCH_ADMIN_PLACEHOLDER: "Tìm nhân viên, mã yêu cầu...",
    LABEL_ASSET_TYPE: "Loại tài sản",
    LABEL_REASON: "Lý do yêu cầu",
    LABEL_PRIORITY: "Mức độ ưu tiên",
    FILTER_STATUS: "Trạng thái",
    FILTER_PRIORITY: "Ưu tiên",
    TABLE_REQUEST_ID: "Mã YC",
    TABLE_ASSET_TYPE: "Loại tài sản",
    TABLE_EMPLOYEE: "Nhân viên",
    TABLE_REASON: "Lý do",
    TABLE_PRIORITY: "Ưu tiên",
    TABLE_STATUS: "Trạng thái",
    TABLE_DATE: "Ngày gửi",
    TABLE_ACTIONS: "Thao tác",
    MODAL_DETAIL_TITLE: "Chi tiết yêu cầu tài sản",
    LABEL_STATS_TOTAL: "Tổng số",
    LABEL_STATS_PENDING: "Chờ duyệt",
    LABEL_STATS_APPROVED: "Đã duyệt",
    LABEL_STATS_REJECTED: "Từ chối",
    LABEL_PROCESS_HISTORY: "Lịch sử xử lý",
    MSG_SELF_APPROVE_ERROR:
      "Bạn không thể tự duyệt yêu cầu cấp phát của bản thân.",
    PRIORITY_LOW: "Thấp",
    PRIORITY_NORMAL: "Bình thường",
    PRIORITY_HIGH: "Cao",
    PRIORITY_URGENT: "Khẩn cấp",
    PLACEHOLDER_PRIORITY: "Chọn ưu tiên",
    ERR_ASSET_TYPE: "Vui lòng nhập loại tài sản.",
    ERR_REASON: "Vui lòng nhập lý do tối thiểu 10 ký tự.",
    PLACEHOLDER_STATUS: "Trạng thái",
    UNIT_REQUEST: "yêu cầu",
    EMPTY_TITLE: "Chưa có yêu cầu nào",
    EMPTY_DESC: "Bạn chưa gửi yêu cầu cấp phát tài sản nào.",
    EMPTY_ADMIN_DESC: "Không tìm thấy yêu cầu nào phù hợp",
    LABEL_REVIEWED_BY: "Người xử lý",
    LABEL_REVIEWED_AT: "Ngày xử lý",
    LABEL_REVIEW_NOTE: "Ghi chú xử lý",
    PLACEHOLDER_REVIEW_NOTE: "Ghi chú xử lý / Phản hồi",
    PLACEHOLDER_NOTE: "Nhập ghi chú phản hồi cho nhân viên...",
    MSG_APPROVE_SUCCESS: "Đã duyệt yêu cầu thành công",
    MSG_REJECT_SUCCESS: "Đã từ chối yêu cầu",
    MSG_CANCEL_SUCCESS: "Đã hủy yêu cầu!",
    MSG_PROCESS_ERROR: "Có lỗi khi xử lý yêu cầu",
    MSG_FETCH_DETAIL_ERROR: "Không thể tải chi tiết yêu cầu",
    MSG_FETCH_ERROR: "Không thể tải danh sách yêu cầu",
    MSG_FETCH_DETAIL_ERROR_LONG: "Không thể tải chi tiết yêu cầu.",
    MODAL_CREATE_TITLE: "Tạo yêu cầu tài sản",
    MODAL_CREATE_DESC:
      "Vui lòng nhập thông tin loại tài sản bạn cần và lý do chi tiết.",
    PLACEHOLDER_TYPE: "VD: Laptop, Màn hình, Bàn phím...",
    PLACEHOLDER_REASON:
      "VD: Máy tính hiện tại quá chậm, cần màn hình phụ để làm việc hiệu quả hơn...",
    BTN_SUBMIT: "Gửi yêu cầu",
    BTN_CANCEL: "Hủy",
    BTN_APPROVE: "Duyệt",
    BTN_REJECT: "Từ chối",
    MSG_CREATE_SUCCESS: "Yêu cầu đã được gửi thành công!",
    MSG_CREATE_ERROR: "Không thể tạo yêu cầu. Vui lòng thử lại.",
    PRIORITY_OPTIONS: {
      LOW: "Thấp",
      NORMAL: "Bình thường",
      HIGH: "Cao",
      URGENT: "Khẩn cấp",
    },
    LABEL_DEVICE_CODE: "Mã thiết bị",
    LABEL_ASSET_TAG_SHORT: "Tag tài sản",
  },

  // Review Adjustment Sheet
  REVIEW: {
    DETAIL_TITLE: "Chi tiết yêu cầu",
    ADJUSTMENT_DETAIL: "Chi tiết điều chỉnh",
    REASON_REQUEST: "Lý do yêu cầu",
    INTERNAL_NOTES: "Ghi chú nội bộ",
    PLACEHOLDER_NOTE: "Thêm ghi chú cho phòng nhân sự...",
    ACTIVITY_HISTORY: "Lịch sử hoạt động",
    BTN_APPROVE: "Duyệt yêu cầu",
    BTN_REJECT: "Từ chối",
    BTN_RETURN: "Gửi lại",
    SHEET_TITLE: "Phê duyệt yêu cầu",
    URGENT: "KHẨN CẤP",
    ORIGINAL: "Ban đầu",
    PROPOSED: "Đề xuất",
    LABEL_DATE_ADJ: "Ngày điều chỉnh: ",
  },

  // Camera
  CAMERA: {
    LOADING: "Đang khởi động camera…",
    ERROR: "Lỗi camera",
    PERMISSION_DENIED: "Không thể truy cập camera",
    TITLE: "Xác nhận danh tính",
    DESC: "Hãy chụp ảnh và cho phép truy cập vị trí để tiếp tục.",
    LOCATION_LOADING: "Đang lấy vị trí…",
    NO_LOCATION: "Không có vị trí",
    BTN_CAPTURE: "Chụp ảnh",
    BTN_RETAKE: "Chụp lại",
    BTN_RETRY: "Thử lại",
  },

  // Login
  LOGIN: {
    TITLE: "Chào mừng trở lại",
    TITLE_SHORT: "Đăng nhập",
    DESC: "Đăng nhập vào hệ thống quản lý nhân sự EMS",
    LABEL_EMAIL: "Email",
    PLACEHOLDER_EMAIL: "admin@ems.com",
    LABEL_PASSWORD: "Mật khẩu",
    PLACEHOLDER_PASSWORD: "••••••••",
    LABEL_REMEMBER: "Ghi nhớ đăng nhập",
    BTN_LOGIN: "Đăng nhập",
    BTN_PROCESSING: "Đang xử lý...",
    LINK_FORGOT: "Quên mật khẩu?",
    LOADING: "Đang xác thực...",
    SUCCESS: "Đăng nhập thành công!",
  },

  // Two Factor Login
  TWO_FACTOR_LOGIN: {
    TITLE: "Xác thực 2 lớp",
    DESC: "Nhập mã OTP 6 số từ ứng dụng xác thực của bạn để tiếp tục.",
    LABEL_OTP: "Mã xác thực (OTP)",
    PLACEHOLDER_OTP: "000000",
    BTN_VERIFY: "Xác thực & Đăng nhập",
    BTN_BACK: "Quay lại",
    TOAST_INVALID: "Mã OTP không đúng. Vui lòng thử lại.",
  },

  // Profile Reset Password
  PROFILE_RESET: {
    BTN_RESET_PASSWORD: "Thiết lập lại mật khẩu",
    DIALOG_TITLE: "Đổi mật khẩu",
    DIALOG_DESC: "Đã gửi mã 6 số đến email của bạn",
    BTN_SEND_OTP: "Gửi mã OTP",
    BTN_RESEND_OTP: "Gửi lại mã mới",
    BTN_SENDING_OTP: "Đang gửi...",
    TOAST_SEND_OTP_SUCCESS: "Mã OTP đã được gửi đến email của bạn!",
    TOAST_SEND_OTP_LOADING: "Đang gửi mã OTP...",
    TOAST_EMAIL_REQUIRED: "Vui lòng nhập email công ty",
  },

  // Forgot Password
  FORGOT_PASSWORD: {
    TITLE: "Quên mật khẩu?",
    TITLE_OTP: "Xác thực OTP",
    TITLE_NEW_PASS: "Đặt mật khẩu mới",
    DESC: "Nhập email để nhận mã xác thực.",
    DESC_SUCCESS: "Chúng tôi đã gửi mã OTP đến email của bạn",
    DESC_OTP_PREFIX: "Đã gửi mã 6 số đến ",
    DESC_NEW_PASS: "Nhập mật khẩu mới cho tài khoản của bạn.",
    LABEL_EMAIL: "Email công ty",
    PLACEHOLDER_EMAIL: "user@example.com",
    PLACEHOLDER_OTP: "000000",
    PLACEHOLDER_PASSWORD: "Tối thiểu 8 ký tự",
    PLACEHOLDER_CONFIRM: "Nhập lại mật khẩu mới",
    PLACEHOLDER_CURRENT_PASSWORD: "Nhập mật khẩu hiện tại",
    BTN_SEND: "Gửi mã xác thực",
    BTN_SENDING: "Đang gửi...",
    BTN_VERIFY: "Xác nhận & Đặt mật khẩu",
    BTN_VERIFYING: "Đang xử lý...",
    BTN_RESET: "Đặt lại mật khẩu",
    BTN_RESEND: "Gửi lại mã mới",
    BTN_RESENDING: "Đang gửi lại...",
    BTN_GO_LOGIN: "Về trang đăng nhập",
    BTN_BACK_EMAIL: "Nhập lại Email",
    LABEL_OTP: "Mã OTP",
    LABEL_PASSWORD: "Mật khẩu mới",
    LABEL_CONFIRM: "Xác nhận mật khẩu",
    LABEL_CURRENT_PASSWORD: "Mật khẩu hiện tại",
    OTP_VALID_SUFFIX: "Mã hết hạn sau ",
    OTP_EXPIRED: "Mã đã hết hạn",
    LINK_BACK_LOGIN: "Quay lại đăng nhập",
    SUCCESS_TITLE: "Đặt lại mật khẩu thành công!",
    SUCCESS_DESC: "Bạn có thể đăng nhập bằng mật khẩu mới.",
    // Toast messages
    TOAST_OTP_SENT: "Mã OTP đã được gửi đến email của bạn.",
    TOAST_OTP_RESENT: "Đã gửi lại mã OTP mới.",
    TOAST_OTP_INVALID: "Mã OTP không hợp lệ hoặc đã hết hạn.",
    TOAST_SEND_ERROR: "Không thể gửi mã. Vui lòng thử lại.",
    TOAST_RESEND_ERROR: "Không thể gửi lại mã. Vui lòng thử lại.",
    TOAST_SUCCESS: "Đặt lại mật khẩu thành công!",
    TOAST_LOADING: "Đang gửi mã OTP...",
    LOADING: "Đang cập nhật mật khẩu...",
    TOAST_OTP_EXPIRED: "Mã xác thực đã hết hạn",
  },
  CHANGE_PASSWORD: {
    NEWPASSWORD_DIFFERENT_CURRENT: "Mật khẩu mới phải khác mật khẩu hiện tại.",
  },

  // Common Translations
  COMMON_EN: {
    SELECT_TYPE: "Chọn loại...",
    SELECT_STATUS: "Chọn trạng thái...",
    UNKNOWN: "Không xác định",
    NO_DATA: "Không có dữ liệu",
  },

  // Common Constants
  COMMON: {
    EMPTY_VALUE: "—",
    HOURS_UNIT: "h",
    DAYS_UNIT: "ngày",
    METHOD_CAMERA_GPS: "Camera+GPS",
    METHOD_MANUAL: "Thủ công",
    CURRENCY_VND: " VNĐ",
    DEFAULT_PAGE_SIZE: 10,
    MAX_SCORE: 100,
    YES: "Có",
    NO: "Không",
    PREV: "Trước",
    NEXT: "Sau",
    TOTAL: "Tổng cộng",
    RESULT: "kết quả",
    PAGE: "Trang",
  },

  // KPI/OKR Management
  KPI_OKR: {
    TITLE: "Thiết lập KPI/OKR",
    TOTAL_WEIGHT: "Tổng trọng số hiện tại",
    TARGET_COUNT: "MỤC TIÊU ĐÃ LẬP",
    DAYS_LEFT: "THỜI GIAN CÒN LẠI",
    DAYS: " Ngày",
    WARNING_WEIGHT: "Chưa hoàn tất (Cần 100% để kích hoạt bộ chỉ số)",
    WARNING_WEIGHT_ACTIVE: "Đã đạt 100% — Bộ chỉ số đã kích hoạt",
    SEARCH_PLACEHOLDER: "Tìm kiếm mục tiêu...",
    BTN_FILTER: "Bộ lọc",
    BTN_ADD: "Thêm mục tiêu mới",
    TABLE_NAME: "TÊN MỤC TIÊU",
    TABLE_TYPE: "LOẠI",
    TABLE_WEIGHT: "TRỌNG SỐ (%)",
    TABLE_METRIC: "CHỈ SỐ ĐO LƯỜNG",
    TABLE_TARGET: "MỤC TIÊU",
    TABLE_ACTUAL: "THỰC TẾ",
    TABLE_RATIO: "TỈ LỆ (%)",
    TABLE_ACTIONS: "HÀNH ĐỘNG",
    TABLE_STATUS: "TRẠNG THÁI",
    PAGINATION_SHOW: "Hiển thị ",
    PAGINATION_OF: " / ",
    PAGINATION_ITEMS: " mục tiêu",
    MODAL_TITLE: "Thêm mục tiêu mới",
    MODAL_DESC:
      "Thiết lập KPI hoặc OKR mới cho kỳ đánh giá này để theo dõi tiến độ công việc.",
    LABEL_NAME: "Tên mục tiêu",
    PLACEHOLDER_NAME: "Nhập tên mục tiêu cụ thể (VD: Tăng doanh thu tháng 10)",
    LABEL_TYPE: "Loại",
    LABEL_SCOPE_TYPE: "Phạm vi",
    LABEL_PERIOD_START: "Bắt đầu kỳ",
    LABEL_PERIOD_END: "Kết thúc kỳ",
    LABEL_METRIC: "Chỉ số đo lường",
    PLACEHOLDER_METRIC: "Chọn đơn vị đo lường",
    METRIC_PERCENT: "Phần trăm (%)",
    METRIC_CURRENCY: "Số tiền (VNĐ)",
    METRIC_NUMBER: "Số lượng",
    LABEL_TARGET: "Giá trị mục tiêu",
    LABEL_WEIGHT: "Trọng số",
    LABEL_DESC: "Mô tả chi tiết (không bắt buộc)",
    PLACEHOLDER_DESC: "Nhập mô tả cách thức đạt được mục tiêu",
    BTN_CANCEL: "Hủy",
    BTN_SAVE: "Lưu mục tiêu",
    STATUS_ACTIVE: "Đã kích hoạt",
    STATUS_INCOMPLETE: "Chưa hoàn tất",
    LOADING: "Đang tải dữ liệu...",
    EMPTY: 'Chưa có mục tiêu nào. Nhấn "Thêm mục tiêu mới" để bắt đầu.',
  },

  // Payroll Management
  PAYROLL: {
    TITLE: "Quản lý lương",
    DESCRIPTION: "Theo dõi và quản lý bảng lương nhân viên.",
    FILTER_PERIOD: "Kỳ lương",
    FILTER_DEPARTMENT: "Phòng ban",
    FILTER_EMPLOYEE: "Tên nhân viên",
    FILTER_EMPLOYEE_PLACEHOLDER: "Tìm kiếm nhân viên...",
    FILTER_STATUS: "Trạng thái",
    BTN_RESET: "Đặt lại",
    BTN_CLEAR: "Xóa bộ lọc",
    BTN_SEARCH: "Tìm kiếm",
    TABLE_EMPLOYEE: "Nhân viên",
    TABLE_DEPARTMENT: "Phòng ban/Vị trí",
    TABLE_BASE_SALARY: "Lương cơ bản",
    TABLE_ALLOWANCE: "Phụ cấp",
    TABLE_BONUS: "Thưởng",
    TABLE_DEDUCTION: "Khấu trừ",
    TABLE_NET: "Thực lĩnh",
    TABLE_STATUS: "Trạng thái",
    TABLE_ACTIONS: "Thao tác",
    PAGINATION_TEXT:
      "Hiển thị {{from}} đến {{to}} trong tổng số {{total}} nhân viên",
  },

  // Salary Component
  SALARY_COMPONENT: {
    MSG_PROCESS_ERROR: "Không thể xử lý yêu cầu. Vui lòng thử lại.",
  },

  // Member List (Danh sách thành viên)
  MEMBER_LIST: {
    TITLE: "Danh sách thành viên",
    DESC: "Quản lý và đánh giá năng lực nhân viên hệ thống",
    BTN_ADD: "Thêm thành viên",
    SEARCH_PLACEHOLDER: "Tìm kiếm nhân viên...",
    TABLE_NAME: "Họ và tên",
    TABLE_ROLE: "Chức vụ",
    TABLE_SKILLS: "Kỹ năng",
    TABLE_ACTIONS: "Thao tác",
    BTN_VIEW_EVALUATION: "Xem chi tiết",
    BTN_EVALUATE: "Đánh giá",
    PAGINATION_SHOW: "Hiển thị",
    PAGINATION_IN: "trong số",
    PAGINATION_MEMBERS: "thành viên",
    LOADING_LIST: "Đang tải danh sách thành viên…",
    ERROR_FETCH: "Không thể tải danh sách. Vui lòng thử lại.",
    EMPTY_LIST: "Không tìm thấy thành viên nào.",
    PROMPT_PERIOD: "Nhập kỳ đánh giá (ví dụ: 2026-Q1, 2026-H1, 2026-ANNUAL)",
    ERROR_PERIOD_FORMAT: "Kỳ đánh giá không đúng định dạng",
    ERROR_OPEN_CYCLE: "Lỗi khi mở đợt đánh giá",
    BTN_OPEN_CYCLE_PENDING: "Đang mở đợt...",
    BTN_OPEN_CYCLE: "Mở đợt đánh giá (3 ngày)",
    ERROR_NO_ACTIVE_CYCLE: "Hiện chưa có đợt đánh giá đang mở",
    ACTIVE_CYCLE_INFO: (period: string, date: string) =>
      `Đợt đánh giá ${period} đang mở đến ${date}`,
    SUCCESS_OPEN_CYCLE: "Đã mở đợt đánh giá thành công!",
    PERIOD_EXAMPLE: "Ví dụ: 2026-Q1, 2026-ANNUAL...",
    EXPORT_PDF_TITLE: "Phiếu đánh giá năng lực",
    EXPORT_PDF_DATE: "Ngày xuất",
    EXPORT_PDF_NAME: "Họ và tên",
    EXPORT_PDF_EMAIL: "Email",
    EXPORT_PDF_ROLE: "Vị trí",
    EXPORT_PDF_EMP_CODE: "Mã NV",
    EXPORT_PDF_TOTAL_SCORE: "Tổng điểm",
    EXPORT_PDF_RANK: "Xếp loại",
    EXPORT_PDF_COMMENT: "Nhận xét",

    // Evaluation sheet
    SHEET_TITLE: "Chi tiết đánh giá",
    SHEET_DESC: "Xem đánh giá năng lực và nhận xét từ quản lý.",
    SHEET_TOTAL_SCORE: "Tổng điểm",
    SHEET_RANK: "Xếp loại",
    SHEET_CRITERIA: "Chi tiết tiêu chí",
    SHEET_CRITERIA_EXPERTISE: "Chuyên môn",
    SHEET_CRITERIA_COMMUNICATION: "Giao tiếp",
    SHEET_CRITERIA_ATTITUDE: "Thái độ",
    SHEET_CRITERIA_DISCIPLINE: "Kỷ luật",
    SHEET_CRITERIA_DESC_EXPERTISE: "Độ sâu kiến thức và chất lượng đầu ra",
    SHEET_CRITERIA_DESC_COMMUNICATION: "Thuyết trình và hợp tác nhóm",
    SHEET_CRITERIA_DESC_ATTITUDE: "Tinh thần trách nhiệm và chủ động",
    SHEET_RATING_EXCELLENT: "Xuất sắc",
    SHEET_RATING_GOOD: "Tốt",
    SHEET_RATING_NEEDS_IMPROVEMENT: "Cần cải thiện",
    SHEET_RANK_A: "Xuất sắc - A",
    SHEET_RANK_B: "Tốt - B",
    SHEET_RANK_C: "Trung bình - C",
    SHEET_PERIOD_LABEL: "Kỳ",
    SHEET_SCORE_LABEL: "Điểm",
    SHEET_COMMENT_TITLE: "Nhận xét từ quản lý",
    SHEET_COMMENT_TEXT:
      "Nhân viên có tinh thần trách nhiệm cao, hoàn thành các task phân tích dữ liệu đúng hạn với độ chính xác tuyệt đối. Cần cải thiện thêm khả năng thuyết trình trước đám đông và chủ động hơn trong việc đề xuất giải pháp cải tiến quy trình phòng ban.",
    SHEET_COMMENT_AUTHOR_NAME: "Lê Hoàng",
    SHEET_COMMENT_AUTHOR_ROLE: "Trưởng phòng Phân tích",
    SHEET_EXPORT: "Xuất file báo cáo (PDF)",
    SHEET_CLOSE: "Đóng",
    SHEET_EDIT_TITLE: "Đánh giá năng lực",
    SHEET_ROLE_LABEL: "Vai trò đánh giá",
    SHEET_ROLE_DESC: "Bạn đang thực hiện đánh giá 360 độ với vai trò: ",
    SHEET_ROLE_CHANGE: "Thay đổi",
    SHEET_EMP_CODE_LABEL: "Mã NV",
    SHEET_EDIT_DESC: "Nhập điểm để hoàn thành đánh giá nhân viên.",
    SHEET_CRITERIA_REQUIRED: "Bắt buộc",
    SHEET_SCORE_PLACEHOLDER: "0 - 100",
    SHEET_COMMENT_PLACEHOLDER: "Nhập ý kiến đóng góp cho nhân viên tại đây...",
    SHEET_COMMENT_HELPER:
      "Mọi phản hồi của bạn sẽ được bảo mật và sử dụng cho mục đích cải thiện năng lực đội ngũ.",
    SHEET_SUBMIT: "Gửi đánh giá",
    SHEET_SUBMITTING: "Đang gửi...",
    SHEET_SUBMIT_SUCCESS: "Đã gửi đánh giá thành công!",
    SHEET_SUBMIT_ERROR: "Gửi đánh giá thất bại. Vui lòng thử lại.",
    SHEET_EXPORT_SUCCESS: "Xuất báo cáo thành công!",
    SHEET_EXPORT_POPUP_ERROR: "Vui lòng cho phép mở popup để xuất PDF",
    SHEET_CANCEL: "Hủy",

    // Extended for 360° manager feature
    BTN_SELF_VIEW: "Xem đánh giá của tôi",
    BTN_SELF_EVALUATE: "Tự đánh giá",
    REVIEW_PERIOD_PREFIX: "Kỳ đánh giá hiện tại:",
    DEFAULT_ROLE: "—",
    TAG_SELF: "Bạn",
    SHEET_WEIGHT_NOTE:
      "Trọng số: Quản lý 40% · Đồng nghiệp 30% · Tự đánh giá 20% · Upward 10%",
    SHEET_NO_REVIEWS: "Chưa có đánh giá nào trong kỳ này.",
    SHEET_LOADING: "Đang tải...",
    SHEET_REVIEW_PERIOD: "Kỳ đánh giá",
    SHEET_TAB_360: "Đánh giá 360°",
    SHEET_TAB_MEETING: "One-on-One",
    SHEET_RATING_AVERAGE: "Khá",
    BREAKDOWN_SCORE_SUFFIX: "/100",
    BREAKDOWN_EXPAND: "Chi tiết",
    BREAKDOWN_COLLAPSE: "Thu gọn",
    BREAKDOWN_EXPERTISE: "Chuyên môn",
    BREAKDOWN_COMMUNICATION: "Giao tiếp",
    BREAKDOWN_ATTITUDE: "Thái độ",
    TYPE_MANAGER: "Quản lý",
    TYPE_SELF: "Tự đánh giá",
    TYPE_PEER: "Đồng nghiệp",
    TYPE_UPWARD: "Upward",
    MEETING_HISTORY_PREFIX: "Lịch sử buổi 1-on-1 với",
    MEETING_BTN_NEW_NOTE: "Ghi chú mới",
    MEETING_NEW_TITLE: "Buổi họp mới",
    MEETING_PLACEHOLDER_AGENDA: "Chủ đề / agenda",
    MEETING_PLACEHOLDER_NOTES: "Nội dung trao đổi...",
    MEETING_PLACEHOLDER_ACTIONS: "Action items...",
    MEETING_BTN_SAVE: "Lưu bản ghi",
    MEETING_BTN_SAVING: "Đang lưu...",
    MEETING_BTN_CANCEL: "Hủy",
    MEETING_LOADING: "Đang tải...",
    MEETING_EMPTY: "Chưa có buổi 1-on-1 nào.",
    MEETING_ACTION_ITEMS_LABEL: "Action items",
    MEETING_SELECT_DATE_ERROR: "Chọn ngày họp",
    RANK_A: "Xuất sắc - A",
    RANK_B: "Tốt - B",
    RANK_C: "Khá - C",
    RANK_D: "Trung bình - D",
    RANK_E: "Cần cải thiện - E",
    RANK_NONE: "—",
    AGG_MANAGER: "Quản lý",
    AGG_SELF: "Tự đánh giá",
    AGG_PEER_PREFIX: "Đồng nghiệp",
    AGG_UPWARD: "Upward",
    DEFAULT_PAGE_SIZE: 10,
    SCORE_DENOMINATOR: "/100",
    SCORE_DENOM_SPACED: "/ 100",
    DASH_EMPTY: "—",
  },
  // Validation Messages
  VALIDATION: {
    EMAIL_REQUIRED: "Vui lòng nhập Username/Email",
    PASSWORD_REQUIRED: "Vui lòng nhập mật khẩu",
    EMAIL_PASSWORD_INVALID: "Email hoặc mật khẩu không chính xác",
  },

  // Symbols & Icons
  SYMBOLS: {
    COLON: ": ",
    PENCIL: "edit",
    CLOSE: "close",
    PERSON: "person",
    CAMERA: "camera",
    BADGE: "badge",
    INFO: "info",
    SLASH: " / ",
    HASH: "#",
    PAREN_OPEN: "(",
    PAREN_CLOSE: ")",
    DASH: " – ",
    EM_DASH: " — ",
    WARNING: "⚠️",
    BULLET: " • ",
    QUOTE: '"',
    ARROW_RIGHT: "→",
    PERCENT: "%",
    EXCLAMATION: "!",
    SPACE: " ",
    REQUIRED: " *",
    ASTERISK: "*",
    LEVEL: "Lv",
  },
  SECURITY: {
    FORBIDDEN_CODE: "403",
    FORBIDDEN_TITLE: "Truy cập bị từ chối",
    FORBIDDEN_DESC: "Bạn không có quyền truy cập vào trang này.",
    GO_HOME: "Về trang chủ",
  },
};

...
import type { ReviewBreakdown } from "@/services/memberService";
...
function typeLabel(type: string): string {
  const map: Record<string, string> = {
    MANAGER: t.TYPE_MANAGER,
    SELF: t.TYPE_SELF,
    PEER: t.TYPE_PEER,
    UPWARD: t.TYPE_UPWARD,
  };
  ...
  return (
    {
      ...
      UPWARD: "bg-pink-100 text-pink-800",
  ...
  return (
    <div className="flex items-center justify-between text-sm py-1">
      ...
      <div className="flex items-center gap-2">
        ...
        <span
          className={`text-[10px] px-1.5 py-0.5 rounded-md font-bold ${getRatingClass(score)}`}
        >
          {getRating(score)}
        </span>
      </div>
  ...
  return (
    <div className="rounded-lg border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span
            className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${typeBadge(r.reviewType)}`}
          >
            {label}
          </span>
        ...
        <div className="flex items-center gap-2">
          <span className="font-bold text-primary text-sm">
            {r.totalScore}
            {t.BREAKDOWN_SCORE_SUFFIX}
          </span>
          <button
            className="text-xs text-slate-400 hover:text-primary"
            onClick={() => setOpen((v) => !v)}
          >
            {open ? t.BREAKDOWN_COLLAPSE : t.BREAKDOWN_EXPAND}
          </button>
        </div>
      </div>
      {open && (
        <div className="space-y-1 pt-1 border-t border-slate-50 dark:border-slate-800">
          <ScoreRow label={t.BREAKDOWN_EXPERTISE} score={r.expertiseScore} />
          <ScoreRow
            label={t.BREAKDOWN_COMMUNICATION}
            score={r.communicationScore}
          />
          <ScoreRow label={t.BREAKDOWN_ATTITUDE} score={r.attitudeScore} />
          {r.comment && (
            <p className="text-xs italic text-slate-500 pt-1 border-t border-slate-50 dark:border-slate-800">
              {r.comment}
            </p>
          )}
        </div>
...
function AggregateView({ member }: { member: Member }) {
  ...
  return (
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <span className="text-xs text-slate-500">{t.SHEET_REVIEW_PERIOD}</span>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-1 rounded-lg p-3 bg-white dark:bg-slate-900 shadow-sm border-l-4 border-emerald-500">
          ...
          <span
            className={`text-2xl font-bold ${rankClass(agg.overallScore ?? 0, has)}`}
          >
            {agg.overallRank}
          </span>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        {[
          {
            ok: agg.hasManagerReview,
            label: t.AGG_MANAGER,
            cls: "bg-amber-100 text-amber-800",
          },
          {
            ok: agg.hasSelfReview,
            label: t.AGG_SELF,
            cls: "bg-purple-100 text-purple-800",
          },
          {
            ok: agg.peerReviewCount > 0,
            label: `${t.AGG_PEER_PREFIX} (${agg.peerReviewCount})`,
            cls: "bg-blue-100 text-blue-800",
          },
          {
            ok: agg.hasUpwardReview,
            label: t.AGG_UPWARD,
          ...
          <span
            key={label}
            className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${ok ? cls : "bg-slate-100 text-slate-400"}`}
          >
            {ok ? "✓" : "○"} {label}
          </span>
        ))}
      </div>
      <p className="text-[10px] text-slate-400 italic">{t.SHEET_WEIGHT_NOTE}</p>
      <div className="space-y-2">
        {agg.managerReview && (
          <BreakdownCard r={agg.managerReview} label={typeLabel("MANAGER")} />
        )}
        {agg.selfReview && (
          <BreakdownCard r={agg.selfReview} label={typeLabel("SELF")} />
        )}
        {agg.peerReviews.map((r) => (
          <BreakdownCard key={r.reviewId} r={r} label={typeLabel("PEER")} />
        ))}
        {agg.upwardReview && (
          <BreakdownCard r={agg.upwardReview} label={typeLabel("UPWARD")} />
        )}
...
function OneOnOneView({ member }: { member: Member }) {
  ...
  const handleCreate = async () => {
    if (!form.meetingDate) {
      toast.error(t.MEETING_SELECT_DATE_ERROR);
      return;
    }
    ...
    <div className="p-4 space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-xs text-slate-500">
          {t.MEETING_HISTORY_PREFIX} {member.name}
        </p>
        <Button
          size="sm"
          variant="outline"
      ...
      {showForm && (
        <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-3 space-y-2">
          ...
          />
          <input
            placeholder={t.MEETING_PLACEHOLDER_AGENDA}
            value={form.agenda}
          ...
          />
          <textarea
            placeholder={t.MEETING_PLACEHOLDER_ACTIONS}
            rows={2}
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {meetings.map((m) => (
            <div
              key={m.id}
              className="rounded-lg border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-3 space-y-1"
            >
              <div className="flex items-center justify-between">
...
function EditForm({
  ...
  MemberEvaluationSheetProps,
  "member" | "initialScores" | "initialComment" | "onOpenChange" | "onSubmit"
> & { member: Member }) {
  ...
  const criteria = [
    {
      ...
      desc: t.SHEET_CRITERIA_DESC_EXPERTISE,
      icon: <Star className="w-4 h-4" />,
    },
    {
      key: "communication",
      label: t.SHEET_CRITERIA_COMMUNICATION,
      desc: t.SHEET_CRITERIA_DESC_COMMUNICATION,
      icon: <MessageCircle className="w-4 h-4" />,
    },
    {
      key: "attitude",
      label: t.SHEET_CRITERIA_ATTITUDE,
      desc: t.SHEET_CRITERIA_DESC_ATTITUDE,
      ...
      <div className="p-4 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="flex flex-col gap-1 rounded-lg p-3 bg-white dark:bg-slate-900 shadow-sm border-l-4 border-emerald-500">
            ...
            <span
              className={`text-2xl font-bold ${rankClass(totalScore, hasAnyScore)}`}
            >
              {resolveRank(totalScore, hasAnyScore)}
            </span>
          </div>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`inline-block px-2 py-0.5 rounded-md ${getRatingClass(score)} text-[10px] font-bold`}
                  >
                    {getRating(score)}
                  </span>
                </div>
...
export function MemberEvaluationSheet({
  ...
  const prevMemberRef = useRef<number | null>(null);
      ...
      <SheetContent className="w-full sm:max-w-md p-0 flex flex-col gap-0 border-l shadow-2xl">
        ...
        {!canEdit && (
          <div className="flex border-b bg-muted/5">
            {(
              [
                [t.SHEET_TAB_360, t.SHEET_TAB_360, Users],
                [t.SHEET_TAB_MEETING, t.SHEET_TAB_MEETING, ClipboardList],
              ] as const
            ).map(([key, label, Icon]) => (
              <button
                key={key}
                className={`flex-1 py-2.5 text-sm font-medium flex items-center justify-center gap-1.5 transition-colors ${tab === (key === t.SHEET_TAB_360 ? "360" : "meeting") ? "text-primary border-b-2 border-primary" : "text-muted-foreground hover:text-foreground"}`}
                onClick={() =>
                  setTab(key === t.SHEET_TAB_360 ? "360" : "meeting")
                }
              >

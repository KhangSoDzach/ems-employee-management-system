import {
  Check,
  Download,
  Info,
  MessageCircle,
  Star,
  UserCheck,
  X,
} from "lucide-react";
import { useEffect, useMemo, useState } from "react";

import {
  Sheet,
  SheetContent,
  SheetDescription,
  SheetHeader,
  SheetTitle,
} from "@/components/ui/sheet";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import { SYSTEM_MESSAGES } from "@/constants/messages";
import { useLatestReview, useSaveReview } from "../hooks/usePerformanceReview";

export type Member = {
  id: number;
  name: string;
  email: string;
  role: string;
  roleColor: string;
  skills: string[];
  avatar: string;
};

type EvaluationCriterion = {
  key: "expertise" | "communication" | "attitude";
  label: string;
  description: string;
  icon: React.ReactNode;
};

type MemberEvaluationSheetProps = {
  member: Member | null;
  open: boolean;
  mode?: "view" | "edit";
  onOpenChange: (open: boolean) => void;
  onSubmit?: () => void;
};

/** Derives the current review period string, e.g. "2026-Q1" */
function currentReviewPeriod(): string {
  const now = new Date();
  const year = now.getFullYear();
  const month = now.getMonth() + 1; // 1-indexed
  const quarter = Math.ceil(month / 3);
  return `${year}-Q${quarter}`;
}

function getRating(
  value: number,
  t: typeof SYSTEM_MESSAGES.MEMBER_LIST,
): string {
  if (value >= 90) return t.SHEET_RATING_EXCELLENT;
  if (value >= 80) return t.SHEET_RATING_GOOD;
  return t.SHEET_RATING_NEEDS_IMPROVEMENT;
}

function getRatingClass(value: number): string {
  if (value >= 90) return "bg-emerald-100 text-emerald-700";
  if (value >= 80) return "bg-primary/10 text-primary";
  return "bg-amber-100 text-amber-700";
}

function resolveRank(
  score: number,
  t: typeof SYSTEM_MESSAGES.MEMBER_LIST,
): string {
  if (score >= 90) return t.SHEET_RANK_A;
  if (score >= 80) return t.SHEET_RANK_B;
  return t.SHEET_RANK_C;
}

// Static criterion metadata (labels/icons are presentation-only)
function buildCriteria(
  t: typeof SYSTEM_MESSAGES.MEMBER_LIST,
): EvaluationCriterion[] {
  return [
    {
      key: "expertise",
      label: t.SHEET_CRITERIA_EXPERTISE,
      description: t.SHEET_CRITERIA_DESC_EXPERTISE,
      icon: <Star className="w-4 h-4" />,
    },
    {
      key: "communication",
      label: t.SHEET_CRITERIA_COMMUNICATION,
      description: t.SHEET_CRITERIA_DESC_COMMUNICATION,
      icon: <MessageCircle className="w-4 h-4" />,
    },
    {
      key: "attitude",
      label: t.SHEET_CRITERIA_ATTITUDE,
      description: t.SHEET_CRITERIA_DESC_ATTITUDE,
      icon: <UserCheck className="w-4 h-4" />,
    },
  ];
}

export function MemberEvaluationSheet({
  member,
  open,
  mode = "view",
  onOpenChange,
  onSubmit,
}: MemberEvaluationSheetProps) {
  const memberId = member?.id ?? null;

  const t = SYSTEM_MESSAGES.MEMBER_LIST;
  const criteria = useMemo(() => buildCriteria(t), [t]);

  // ── Load the latest review from the API ─────────────────────────────────
  const { data: latestReview, isLoading: reviewLoading } = useLatestReview(
    open ? memberId : null,
  );

  const saveReviewMutation = useSaveReview();

  // ── Editable score state (initialised from API data when it arrives) ─────
  const [scores, setScores] = useState<
    Record<"expertise" | "communication" | "attitude", number>
  >({
    expertise: 0,
    communication: 0,
    attitude: 0,
  });
  const [comment, setComment] = useState<string>("");

  // Sync state when API data arrives or member changes
  useEffect(() => {
    if (latestReview) {
      setScores({
        expertise: latestReview.expertiseScore ?? 0,
        communication: latestReview.communicationScore ?? 0,
        attitude: latestReview.attitudeScore ?? 0,
      });
      setComment(latestReview.comment ?? "");
    } else {
      setScores({ expertise: 0, communication: 0, attitude: 0 });
      setComment("");
    }
  }, [latestReview, memberId]);

  const totalScore = useMemo(() => {
    const values = Object.values(scores);
    if (!values.length) return 0;
    return Math.round(
      values.reduce((acc, cur) => acc + cur, 0) / values.length,
    );
  }, [scores]);

  const rank = resolveRank(totalScore, t);

  const canEdit = mode === "edit";

  const handleExport = () => {
    if (!member) return;
    // Add a temporary title to the document for the filename
    const originalTitle = document.title;
    document.title = `Evaluation_${member.name}_${currentReviewPeriod()}`;
    window.print();
    document.title = originalTitle;
  };

  const handleSubmit = () => {
    if (!member) return;
    saveReviewMutation.mutate(
      {
        revieweeId: member.id,
        reviewType: "MANAGER",
        reviewPeriod: currentReviewPeriod(),
        scores: {
          expertise: scores.expertise,
          communication: scores.communication,
          attitude: scores.attitude,
        },
        comment: comment || undefined,
      },
      {
        onSuccess: () => {
          onSubmit?.();
          onOpenChange(false);
        },
      },
    );
  };

  // ── View-mode score source: prefer API data ──────────────────────────────
  const viewScore = (
    key: "expertise" | "communication" | "attitude",
  ): number => {
    if (key === "expertise") return latestReview?.expertiseScore ?? 0;
    if (key === "communication") return latestReview?.communicationScore ?? 0;
    if (key === "attitude") return latestReview?.attitudeScore ?? 0;
    return 0;
  };

  if (!member) return null;

  return (
    <Sheet
      open={open}
      onOpenChange={(v) => {
        if (!v) onOpenChange(false);
      }}
    >
      <SheetContent className="w-full sm:max-w-md p-0 flex flex-col gap-0 border-l shadow-2xl overflow-hidden print:shadow-none print:border-none print:max-w-none print:w-full print:fixed print:inset-0 print:z-9999 print:bg-white">
        <div className="px-5 py-4 border-b bg-muted/10 relative overflow-hidden print:bg-white print:border-b-2">
          <div className="absolute top-0 right-0 w-32 h-32 bg-primary/5 rounded-full blur-3xl -mr-10 -mt-10" />

          <SheetHeader className="text-left space-y-1">
            <SheetTitle className="text-xl font-bold tracking-tight text-foreground">
              {canEdit ? t.SHEET_EDIT_TITLE : t.SHEET_TITLE}
            </SheetTitle>
            <SheetDescription className="text-sm font-medium text-muted-foreground">
              {canEdit ? t.SHEET_EDIT_DESC : t.SHEET_DESC}
            </SheetDescription>
          </SheetHeader>
        </div>

        <div className="flex-1 overflow-y-auto">
          <div className="p-4 space-y-5">
            {/* Member header */}
            <div className="flex items-center gap-3">
              <Avatar className="w-14 h-14 border">
                <AvatarImage src={member.avatar} alt={member.name} />
                <AvatarFallback>{member.name.charAt(0)}</AvatarFallback>
              </Avatar>
              <div>
                <p className="text-base font-semibold text-foreground">
                  {member.name}
                </p>
                <p className="text-xs text-muted-foreground">{member.email}</p>
                <p className="text-sm font-medium text-primary mt-1">
                  {member.role}
                </p>
                <p className="text-xs text-muted-foreground">
                  {t.SHEET_EMP_CODE_LABEL}: {member.id}
                </p>
              </div>
            </div>

            {/* Role indicator */}
            <div className="bg-primary/5 dark:bg-primary/10 border border-primary/20 rounded-lg p-3 flex items-start gap-3">
              <Info className="text-primary text-xl" />
              <div className="flex-1">
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">
                  {t.SHEET_ROLE_LABEL}
                </p>
                <p className="text-sm text-slate-600 dark:text-slate-400 mt-1">
                  {t.SHEET_ROLE_DESC}
                  <span className="font-bold text-primary">{member.role}</span>
                </p>
              </div>
            </div>

            {/* Loading state */}
            {reviewLoading ? (
              <div className="py-8 text-center text-sm text-muted-foreground">
                Đang tải dữ liệu đánh giá…
              </div>
            ) : (
              <>
                {/* Summary */}
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1 rounded-lg p-3 bg-white dark:bg-slate-900 shadow-sm border-l-4 border-primary">
                    <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider">
                      {t.SHEET_TOTAL_SCORE}
                    </p>
                    <div className="flex items-baseline gap-1">
                      <span className="text-primary text-3xl font-bold leading-tight">
                        {canEdit ? totalScore : (latestReview?.totalScore ?? 0)}
                      </span>
                      <span className="text-slate-400 text-lg font-medium">
                        /100
                      </span>
                    </div>
                  </div>
                  <div className="flex flex-col gap-1 rounded-lg p-3 bg-white dark:bg-slate-900 shadow-sm border-l-4 border-emerald-500">
                    <p className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wider">
                      {t.SHEET_RANK}
                    </p>
                    <div className="flex items-center gap-2">
                      <span className="text-emerald-600 dark:text-emerald-400 text-2xl font-bold leading-tight">
                        {canEdit
                          ? rank
                          : resolveRank(latestReview?.totalScore ?? 0, t)}
                      </span>
                    </div>
                  </div>
                </div>

                {/* Criteria breakdown */}
                <div className="space-y-3">
                  {criteria.map((criterion) => {
                    const score = canEdit
                      ? (scores[criterion.key] ?? 0)
                      : viewScore(criterion.key);
                    const rating = getRating(score, t);
                    const ratingClass = getRatingClass(score);

                    return (
                      <div
                        key={criterion.key}
                        className="flex items-center justify-between p-3 rounded-lg bg-white dark:bg-slate-900 shadow-sm border border-slate-100 dark:border-slate-800"
                      >
                        <div className="flex items-center gap-3">
                          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-primary/10 text-primary">
                            {criterion.icon}
                          </div>
                          <div>
                            <p className="text-slate-900 dark:text-white text-sm font-bold">
                              {criterion.label}
                            </p>
                            <p className="text-slate-500 dark:text-slate-400 text-xs">
                              {criterion.description}
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
                          {canEdit ? (
                            <div className="flex items-center justify-end gap-2">
                              <input
                                type="number"
                                min={0}
                                max={100}
                                value={score}
                                onChange={(e) => {
                                  const next = Math.min(
                                    100,
                                    Math.max(0, Number(e.target.value)),
                                  );
                                  setScores((prev) => ({
                                    ...prev,
                                    [criterion.key]: next,
                                  }));
                                }}
                                placeholder={t.SHEET_SCORE_PLACEHOLDER}
                                className="w-20 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded h-9 px-2 text-sm font-medium text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
                              />
                              <span className="text-sm font-semibold text-slate-500">
                                / 100
                              </span>
                            </div>
                          ) : (
                            <p className="text-slate-900 dark:text-white font-bold text-base">
                              {score}/100
                            </p>
                          )}
                          <span
                            className={`inline-block px-2 py-0.5 rounded-md ${ratingClass} text-[10px] font-bold`}
                          >
                            {rating}
                          </span>
                        </div>
                      </div>
                    );
                  })}
                </div>

                {/* Comment */}
                {canEdit ? (
                  <div className="bg-white dark:bg-slate-900 p-3 rounded-lg shadow-sm border border-slate-100 dark:border-slate-800 space-y-3">
                    <label className="text-sm font-bold text-slate-800 dark:text-slate-200">
                      {t.SHEET_COMMENT_TITLE}
                    </label>
                    <textarea
                      value={comment}
                      onChange={(e) => setComment(e.target.value)}
                      placeholder={t.SHEET_COMMENT_PLACEHOLDER}
                      rows={4}
                      className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded p-4 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all resize-none"
                    />
                    <p className="text-[11px] text-slate-400 italic">
                      {t.SHEET_COMMENT_HELPER}
                    </p>
                  </div>
                ) : (
                  <div className="bg-white dark:bg-slate-900 p-4 rounded-lg shadow-sm border border-slate-100 dark:border-slate-800 italic relative">
                    <span className="absolute -top-3 -left-1 text-4xl text-primary/10 font-serif">
                      "
                    </span>
                    <p className="text-slate-600 dark:text-slate-300 text-sm leading-relaxed">
                      {latestReview?.comment || t.SHEET_COMMENT_TEXT}
                    </p>
                    {latestReview?.reviewerUsername && (
                      <div className="mt-4 flex items-center gap-2 not-italic">
                        <div className="h-8 w-8 rounded-full bg-primary flex items-center justify-center text-white text-[10px] font-bold">
                          {latestReview.reviewerUsername
                            .slice(0, 2)
                            .toUpperCase()}
                        </div>
                        <div>
                          <p className="text-slate-900 dark:text-white text-xs font-bold leading-none">
                            {latestReview.reviewerUsername}
                          </p>
                          <p className="text-slate-500 text-[10px]">
                            {latestReview.reviewPeriod ?? ""}
                          </p>
                        </div>
                      </div>
                    )}
                  </div>
                )}
              </>
            )}
          </div>
        </div>

        {/* Footer actions */}
        <div className="px-5 py-4 border-t bg-muted/20 flex flex-col sm:flex-row gap-3">
          {canEdit ? (
            <>
              <Button
                variant="outline"
                className="flex-1 flex items-center justify-center gap-2 py-3 print:hidden"
                onClick={() => onOpenChange(false)}
                disabled={saveReviewMutation.isPending}
              >
                <X className="w-4 h-4" />
                {t.SHEET_CANCEL}
              </Button>
              <Button
                className="flex-1 flex items-center justify-center gap-2 py-3"
                onClick={handleSubmit}
                disabled={saveReviewMutation.isPending}
              >
                <Check className="w-4 h-4" />
                {saveReviewMutation.isPending ? "Đang lưu…" : t.SHEET_SUBMIT}
              </Button>
            </>
          ) : (
            <>
              <Button
                variant="secondary"
                className="flex-1 flex items-center justify-center gap-2 print:hidden"
                onClick={handleExport}
              >
                <Download className="w-4 h-4" />
                {t.SHEET_EXPORT}
              </Button>
              <Button
                variant="outline"
                className="flex-1 print:hidden"
                onClick={() => onOpenChange(false)}
              >
                {t.SHEET_CLOSE}
              </Button>
            </>
          )}
        </div>
      </SheetContent>
    </Sheet>
  );
}

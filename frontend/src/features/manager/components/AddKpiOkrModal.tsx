...
export function AddKpiOkrModal({ open, onClose, onSuccess }: Props) {
  ...
  const validate = (): boolean => {
    ...
    if (!name.trim()) {
      e.name = "Tên mục tiêu không được để trống";
    }
    if (!targetValue || Number(targetValue) <= 0) {
      e.targetValue = "Giá trị mục tiêu phải lớn hơn 0";
    }
    if (!weight || Number(weight) <= 0 || Number(weight) > 100) {
      e.weight = "Trọng số phải từ 0.01 đến 100";
    }
    if (!periodStart) {
      e.periodStart = "Chọn ngày bắt đầu";
    }
    if (!periodEnd) {
      e.periodEnd = "Chọn ngày kết thúc";
    }
    if (periodStart && periodEnd && periodStart >= periodEnd) {
      e.periodEnd = "Ngày kết thúc phải sau ngày bắt đầu";
    }
    if ((scopeType === "DEPARTMENT" || scopeType === "EMPLOYEE") && !scopeId) {
      e.scopeId = "Bắt buộc khi chọn Phòng ban / Nhân viên";
    }
    setErrors(e);
      ...
      <div className="relative w-full max-w-lg bg-white rounded-lg shadow-2xl border border-slate-200 overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="px-6 py-4 space-y-4 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <select
                ...
                value={type}
                onChange={(e) => setType(e.target.value as KpiType)}
              >
            ...
            <div className="space-y-1">
              ...
              <select
                className={selectCls("metricType")}
                value={metricType}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input
                className={inputCls("targetValue")}
                placeholder="0"
                type="number"
                min="0.01"
                value={targetValue}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {scopeType !== "COMPANY" && (
              <div className="space-y-1">
                <label className="text-sm font-medium text-slate-900">
                  {scopeType === "DEPARTMENT" ? "ID Phòng ban" : "ID Nhân viên"}
                </label>
                <input
                  className={inputCls("scopeId")}
                  placeholder="Nhập ID"
                  type="number"
                  value={scopeId}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input
                className={inputCls("periodStart")}
                type="date"
                value={periodStart}
          ...
          <div className="space-y-1">
            ...
            <textarea
              className="flex min-h-[80px] w-full rounded border border-slate-200 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 bg-slate-50/50 border-t shrink-0">
          <button
            onClick={handleClose}
            disabled={loading}
            className="inline-flex items-center justify-center rounded px-4 py-2 text-sm font-medium border border-slate-200 bg-white hover:bg-slate-100 text-slate-900 disabled:opacity-50"
          >
            {t.BTN_CANCEL}
          </button>
          <button
            ...
            disabled={loading}
            className="inline-flex items-center justify-center rounded px-4 py-2 text-sm font-medium bg-[#e41b21] hover:bg-[#c9181d] text-white disabled:opacity-60 min-w-[120px]"
          >

...
function progressColor(p: number) {
  ...
  if (p >= 60) {
    return { text: "text-orange-500", bar: "bg-orange-400" };
  }
            ...
            <div className="card-soft p-6 flex items-start justify-between">
              <div className="flex-1 space-y-4">
                ...
                <div className="w-full max-w-md bg-muted rounded-full h-2.5 overflow-hidden">
                  <div
                    className={`h-2.5 rounded-full transition-all ${isActive ? "bg-emerald-500" : "bg-red-600"}`}
                    style={{ width: `${weightPct}%` }}
                  />
                </div>
                <div
                  className={`flex items-center gap-1.5 text-sm font-medium w-fit px-3 py-1 rounded-md ${isActive ? "text-emerald-700 bg-emerald-50" : "text-red-600 bg-red-50"}`}
                >
            ...
            <div className="card-soft">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 text-muted-foreground gap-2">
                  ...
                  <p className="text-sm">
                    Chưa có mục tiêu nào. Nhấn "+ Thêm mục tiêu mới" để bắt đầu.
                  </p>
                </div>
              ) : (
                <Table>
                      ...
                      return (
                        <TableRow key={row.id} className="hover:bg-muted/30">
                          ...
                          <TableCell>
                            <Badge
                              variant="outline"
                              className={`border-transparent ${row.type === "KPI" ? "bg-blue-100 text-blue-700" : "bg-purple-100 text-purple-700"}`}
                            >
                              {row.type}
                            </Badge>
                          </TableCell>
                          ...
                          <TableCell className="text-right font-medium">
                            {formatValue(row.actualValue, row.metricType)}
                          </TableCell>

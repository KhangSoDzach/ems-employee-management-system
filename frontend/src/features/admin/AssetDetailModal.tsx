...
export default function AssetDetailModal({ open, assetId, onClose, onChanged }: Props) {
  ...
  useEffect(() => {
    ...
    assetService.getAssetById(assetId)
      .then(setAsset)
      .catch(() => toast.error("Không tải được chi tiết tài sản"))
  ...
  const handleAssign = async () => {
    ...
    try {
      ...
      toast.error("Không thể cấp phát tài sản");
    } finally {
      setAssigning(false);
    }
  };
  ...
  const handleReturn = async () => {
    ...
    try {
      ...
      toast.error("Không thể thu hồi tài sản");
    } finally {
      setReturning(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-slate-900 w-full max-w-5xl max-h-[90vh] overflow-hidden rounded-xl shadow-2xl flex flex-col">
        ...
        <div className="px-6 py-4 border-b flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold">{asset?.name ?? "Chi tiết tài sản"}</h2>
            <p className="text-xs text-slate-500 font-mono">Mã tài sản: {asset?.code ?? "—"}</p>
          </div>
            ...
            <div className="grid grid-cols-12 gap-6">
              ...
              <div className="col-span-12 lg:col-span-4 space-y-6">
                  ...
                  <div className="p-4 space-y-3">
                    {asset.status === "AVAILABLE" && (
                      <button onClick={() => { setShowAssign(true); setEmpSearch(""); }}
                        ...
                        <UserCheck size={16} /> Cấp phát tài sản
                      </button>
                    )}
                ...
                <div className="bg-white dark:bg-slate-900 rounded-lg border shadow-sm p-4">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-slate-500">Bảo hành:</span><span>{asset.warranty ?? "—"}</span></div>
                    <div className="flex justify-between"><span className="text-slate-500">Nhà cung cấp:</span><span>{asset.supplier ?? "—"}</span></div>
                    <div className="flex justify-between"><span className="text-slate-500">Hợp đồng:</span><span className="text-primary">{asset.contract ?? "—"}</span></div>
                  </div>
                </div>
              </div>
              ...
              <div className="col-span-12 lg:col-span-8 space-y-6">
                <div className="bg-white dark:bg-slate-900 rounded-lg border shadow-sm">
                  <div className="px-4 py-3 border-b"><h3 className="font-bold text-sm">Thông tin cơ bản</h3></div>
                  <div className="p-5 grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                    <div><p className="text-slate-400 text-xs">Mã tài sản</p><p className="font-medium">{asset.code}</p></div>
                    <div><p className="text-slate-400 text-xs">Loại</p><p className="font-medium">{asset.type ?? "—"}</p></div>
                    <div><p className="text-slate-400 text-xs">Giá trị</p><p className="font-medium">{asset.value ?? "—"}</p></div>
                    <div><p className="text-slate-400 text-xs">Ngày mua</p><p className="font-medium">{asset.purchaseDate ?? "—"}</p></div>
                    <div>
                    ...
                    <div>
                      <p className="text-slate-400 text-xs flex items-center gap-1"><MapPin size={12} /> Vị trí / Người dùng</p>
                      <p className="font-medium text-slate-700">{asset.location ?? "—"}</p>
                    </div>
                    {asset.description && (
                      <div className="col-span-2 md:col-span-3 pt-3 border-t">
                        <p className="text-slate-400 text-xs mb-2">Mô tả chi tiết</p>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-lg border shadow-sm">
                  ...
                  <div className="p-5 space-y-4 text-sm">
                    ...
                    )) : <p className="text-slate-400 text-xs">Chưa có lịch sử</p>}
                  </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-lg">Cấp phát tài sản</h3>
              <button onClick={() => setShowAssign(false)}><X size={18} /></button>
            </div>
            <div className="relative">
              <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
              <input placeholder="Tìm nhân viên..." value={empSearch}
            ...
            <textarea rows={2} placeholder="Ghi chú (tuỳ chọn)" value={assignNote}
              onChange={(e) => setAssignNote(e.target.value)}
              className="w-full border rounded-md px-3 py-2 text-sm resize-none" />
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowAssign(false)} className="px-4 py-2 text-sm border rounded-md">Hủy</button>
              <button onClick={handleAssign} disabled={!selectedEmp || assigning}
                ...
                {assigning && <Loader2 size={14} className="animate-spin" />} Xác nhận cấp phát
              </button>
            </div>
          </div>
        </div>
      ...
      {showReturn && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-lg">Thu hồi tài sản</h3>
              <button onClick={() => setShowReturn(false)}><X size={18} /></button>
            </div>
            <div className="space-y-1">
              <label className="text-sm font-medium">Tình trạng khi trả</label>
            ...
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              ...
              Sẵn sàng sử dụng lại (chuyển về Sẵn dùng)
            </label>
            <textarea rows={2} placeholder="Ghi chú (tuỳ chọn)" value={returnNote}
              onChange={(e) => setReturnNote(e.target.value)}
              className="w-full border rounded-md px-3 py-2 text-sm resize-none" />
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowReturn(false)} className="px-4 py-2 text-sm border rounded-md">Hủy</button>
              <button onClick={handleReturn} disabled={returning}
                className="px-4 py-2 text-sm bg-primary text-white rounded-md flex items-center gap-2 disabled:opacity-60">
                {returning && <Loader2 size={14} className="animate-spin" />} Xác nhận thu hồi
              </button>
            </div>
          </div>
        </div>

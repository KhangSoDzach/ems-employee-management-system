...
import { AlertTriangle, CheckCircle2, XCircle, Search, Calendar, Package, FileText, Filter, Eye, MoreHorizontal, BadgeCheck } from "lucide-react"
...
export default function AssetReportManagement() {
    ...
    const handleProcess = async (id: number, note: string, type: 'APPROVE' | 'REJECT') => {
        try {
            if (type === 'APPROVE') {
                ...
                <main className="page-layout-wrapper">
                    ...
                    <div className="page-header mb-6">
                        <div className="space-y-1">
                            <h1 className="page-heading">{SYSTEM_MESSAGES.ASSET_REPORT.TITLE}</h1>
                            <p className="text-muted-foreground text-sm font-medium">{SYSTEM_MESSAGES.ASSET_REPORT.DESC}</p>
                        </div>
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <Button variant="outline" className="h-10 w-10 p-0 rounded-xl bg-white dark:bg-slate-900 border-slate-200 shadow-sm hover:bg-slate-50">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        {[
                            ...
                            <div key={i} className="stat-card-default flex items-center justify-between group transition-all hover:shadow-md">
                                <div>
                                    <p className="stat-label-default">{stat.label}</p>
                                    <p className="stat-value-lg text-slate-900 dark:text-slate-100">{stat.value}</p>
                                </div>
                                <div className={`w-12 h-12 rounded-xl bg-${stat.color}-50 dark:bg-${stat.color}-900/20 flex items-center justify-center group-hover:scale-110 transition-transform`}>
                        ...
                        <Table className="data-table">
                            <TableHeader className="data-table-header">
                                <TableRow className="bg-transparent hover:bg-transparent">
                                    <TableHead className="data-table-header-cell">{SYSTEM_MESSAGES.ASSET_REPORT.TABLE_COLS.CODE}</TableHead>
                                    <TableHead className="data-table-header-cell">{SYSTEM_MESSAGES.ASSET_REPORT.TABLE_COLS.EMPLOYEE}</TableHead>
                                    <TableHead className="data-table-header-cell">{SYSTEM_MESSAGES.ASSET_REPORT.TABLE_COLS.ASSET}</TableHead>
                                    <TableHead className="data-table-header-cell">{SYSTEM_MESSAGES.ASSET_REPORT.TABLE_COLS.ISSUE_TYPE}</TableHead>
                                    <TableHead className="data-table-header-cell">{SYSTEM_MESSAGES.ASSET_REPORT.TABLE_COLS.REPORT_DATE}</TableHead>
                                    <TableHead className="data-table-header-cell text-center">{SYSTEM_MESSAGES.ASSET_REPORT.TABLE_COLS.STATUS}</TableHead>
                                    <TableHead className="data-table-header-cell text-right">{SYSTEM_MESSAGES.ASSET_REPORT.TABLE_COLS.ACTIONS}</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {loading ? (
                                    ...
                                    reports.map((report) => (
                                        <TableRow key={report.id} className="data-table-row group">
                                            <TableCell className="data-table-cell">
                                                <span className="font-black text-primary text-sm tracking-tighter">{SYSTEM_MESSAGES.ASSET_REPORT.TXT_HASH}{report.reportId}</span>
                                            </TableCell>
                                            <TableCell className="data-table-cell">
                                                <div className="flex items-center gap-3">
                                            ...
                                            <TableCell className="data-table-cell text-center">
                                                <div className="flex justify-center">
                                                    <Badge className={`status-badge-pill ${report.statusColor} border-none shadow-sm`}>
                                                        {report.statusLabel}
                                                    </Badge>
                                                </div>
                                            </TableCell>

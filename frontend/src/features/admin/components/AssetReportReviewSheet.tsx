...
import { IncidentReportDetail, ASSET_STATUS_LABELS, AssetStatus, ASSET_CONDITION_LABELS, AssetCondition } from "@/services/assetService"
...
export function AssetReportReviewSheet({
    ...
    const getStatusColor = (color: string) => {
        if (color.includes('emerald') || color.includes('green')) { return 'badge-success'; }
        if (color.includes('rose') || color.includes('red')) { return 'badge-error'; }
        if (color.includes('amber') || color.includes('yellow')) { return 'badge-warning'; }
        return 'badge-gray';
    }
            ...
            >
                ...
                {report ? (
                        ...
                        <div className="flex flex-col gap-8 text-sm">
                                ...
                                <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-5 transition-all">
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{SYSTEM_MESSAGES.ASSET_REPORT.LABEL_ASSET_NAME}</p>
                                            <p className="text-base font-bold text-slate-900 dark:text-slate-100 tracking-tight leading-tight">{report.asset}</p>
                                        </div>
                                        <div className="text-right space-y-1">
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{SYSTEM_MESSAGES.ASSET_REPORT.LABEL_ASSET_STATUS}</p>
                                            <p className="text-[11px] font-bold text-blue-600 bg-blue-50 dark:bg-blue-900/20 px-2 py-0.5 rounded-lg border border-blue-100/50 dark:border-blue-800/50 uppercase tracking-tighter">
                                                {ASSET_STATUS_LABELS[report.assetStatus as AssetStatus] || report.assetStatus}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between pb-4 border-b border-slate-50 dark:border-slate-800/50">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{SYSTEM_MESSAGES.ASSET_REPORT.LABEL_ASSET_CODE}</p>
                                            <p className="text-sm font-mono font-bold text-slate-700 dark:text-slate-300 tracking-tighter">{report.assetCode}</p>
                                        </div>
                                    ...
                                    {(report.status === 'PENDING' || report.status === 'APPROVED') && (
                                        <div className="grid grid-cols-2 gap-6">
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{SYSTEM_MESSAGES.ASSET_REPORT.LABEL_CURRENT_CONDITION}</p>
                                                <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                                                    {ASSET_CONDITION_LABELS[report.assetCondition as AssetCondition] || report.assetCondition}
                                                </p>
                                            </div>
                                            <div className="space-y-1">
                                                <p className="text-[10px] font-bold text-rose-500 uppercase tracking-widest">
                                                    {report.status === 'PENDING' ? SYSTEM_MESSAGES.ASSET_REPORT.LABEL_TARGET_UPDATE : SYSTEM_MESSAGES.ASSET_REPORT.LABEL_UPDATED_TO}
                                                </p>
                                                <p className="text-sm font-bold text-rose-600 dark:text-rose-400">
                                                    {report.incidentType === 'DAMAGED' ? SYSTEM_MESSAGES.ASSET_REPORT.TXT_DAMAGED : SYSTEM_MESSAGES.ASSET_REPORT.TXT_LOST}
                                                </p>
                                            </div>
                                        </div>
                                    )}
                            ...
                            <section className="space-y-4">
                                ...
                                <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-sm space-y-4 transition-all">
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{SYSTEM_MESSAGES.ASSET_REPORT.LABEL_ISSUE_TYPE}</p>
                                        <p className="text-sm font-bold text-slate-900 dark:text-slate-100">{report.incidentTypeLabel}</p>
                                    </div>
                                    <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl italic text-[13px] text-slate-600 dark:text-slate-400 font-medium leading-relaxed border border-slate-100 dark:border-slate-800/50">
                                        <p className="relative z-10">
                                            {SYSTEM_MESSAGES.SYMBOLS.QUOTE}{report.description}{SYSTEM_MESSAGES.SYMBOLS.QUOTE}
                                        </p>
                                    </div>
                                </div>
                            </section>
                            ...
                            <section className="space-y-4">
                                ...
                                {report.attachmentUrl ? (
                                    ...
                                    <div className="h-32 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center bg-white/50 dark:bg-slate-900/50 transition-colors">
                                        ...
                                        <p className="text-[10px] font-bold text-slate-400 dark:text-slate-600 uppercase tracking-widest text-center px-4">{SYSTEM_MESSAGES.ASSET_REPORT.TXT_NO_ATTACHMENT}</p>
                                    </div>
                                )}
                            </section>
                                ...
                                <section className="space-y-4">
                                    ...
                                    <div className="bg-slate-900 dark:bg-black p-6 rounded-2xl space-y-4 shadow-xl border border-slate-800 relative overflow-hidden">
                                        ...
                                        <div className="flex items-center justify-between relative z-10">
                                            <div className="space-y-1">
                                                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{SYSTEM_MESSAGES.ASSET_REPORT.LABEL_PROCESSOR}</p>
                                                <p className="text-sm font-bold text-slate-100 tracking-tight">{report.processedBy}</p>
                                            </div>
                                            <div className="text-right space-y-1">
                                                <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{SYSTEM_MESSAGES.ASSET_REPORT.LABEL_PROCESS_TIME}</p>
                                                <p className="text-[11px] font-semibold text-slate-400">{report.processedAt}</p>
                                            </div>
                                        </div>
                                        
                                        <div className="pt-4 border-t border-slate-800 relative z-10">
                                            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-2">{SYSTEM_MESSAGES.ASSET_REPORT.LABEL_PROCESS_NOTE}</p>
                                            <div className="bg-white/5 rounded-xl p-3 border border-white/5 shadow-inner">
                                                <p className="text-[12px] font-medium text-slate-300 italic leading-relaxed">
                                                    {SYSTEM_MESSAGES.SYMBOLS.QUOTE}{report.processNote || SYSTEM_MESSAGES.ASSET_REPORT.TXT_NO_NOTE}{SYSTEM_MESSAGES.SYMBOLS.QUOTE}
                                                </p>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div className="flex-1 flex items-center justify-center text-muted-foreground p-12 text-center space-y-4 flex-col">
                        ...
                        <p className="text-sm font-medium">{SYSTEM_MESSAGES.STATUS.UNKNOWN}</p>
                    </div>
                )}

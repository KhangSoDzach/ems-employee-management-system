...
import { Eye, Pencil, Trash2, ChevronLeft, ChevronRight, Loader2, UserPlus, Search, FilterX } from "lucide-react";
import { toast } from "sonner";
import { employeeService, EmployeeResponse, PageParams } from "@/services/employeeService";
...
export default function EmployeeManagementPage() {
    ...
    return (
        <SidebarProvider>
            <AppSidebar variant="inset" role="hr" />
            <SidebarInset>
                <SiteHeader />
                <main className="flex flex-1 flex-col p-6 gap-6 bg-gray-50 dark:bg-gray-950">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            >
                                <UserPlus size={18} />
                                <span className="hidden sm:inline">{SYSTEM_MESSAGES.EMPLOYEE.BTN_ADD}</span>
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4">
                        <div className="flex items-center gap-2">
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${statusFilter === ""
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${statusFilter === "ACTIVE"
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 rounded-lg text-sm font-medium transition ${statusFilter === "INACTIVE"
                        ...
                        <div className="overflow-x-auto text-nowrap">
                            <table className="w-full text-sm text-left">
                                <thead className="bg-gray-50/50 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 border-b border-gray-100 dark:border-gray-800">
                                    <tr>
                                        <th className="px-6 py-4 font-semibold">{SYSTEM_MESSAGES.EMPLOYEE.LABEL_EMP_CODE}</th>
                                        <th className="px-6 py-4 font-semibold">{SYSTEM_MESSAGES.EMPLOYEE.LABEL_NAME}</th>
                                        <th className="px-6 py-4 font-semibold">{SYSTEM_MESSAGES.EMPLOYEE.LABEL_DEPARTMENT}</th>
                                        <th className="px-6 py-4 font-semibold">{SYSTEM_MESSAGES.EMPLOYEE.LABEL_POSITION}</th>
                                        <th className="px-6 py-4 font-semibold">{SYSTEM_MESSAGES.LABEL_STATUS}</th>
                                        <th className="px-6 py-4 text-right font-semibold">{SYSTEM_MESSAGES.LABEL_ACTION}</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
                                    {loading ? (
                                        <tr>
                                            <td colSpan={6} className="py-20 text-center text-gray-400">
                                                <div className="flex flex-col items-center gap-3">
                                                    ...
                                                    <p className="animate-pulse">{SYSTEM_MESSAGES.LOADING}</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ) : employees.length === 0 ? (
                                        ...
                                        employees.map((emp) => (
                                            <tr key={emp.id} className="group hover:bg-gray-50/80 dark:hover:bg-gray-800/50 transition-colors">
                                                ...
                                                <td className="px-6 py-4">
                                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold ${emp.status === "ACTIVE"
                                                        ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                                                        : "bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400"
                                                        }`}>
                                                        {emp.status === "ACTIVE" ? SYSTEM_MESSAGES.EMPLOYEE.STATUS_ACTIVE : SYSTEM_MESSAGES.EMPLOYEE.STATUS_INACTIVE}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4">
                                ...
                                {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                                    ...
                                    return (
                                        <button
                                            key={pg}
                                            onClick={() => setPage(pg)}
                                            className={`w-9 h-9 rounded-lg font-bold transition ${pg === page
                ...
                <EmployeeEditModal
                    ...
                    onSuccess={() => { fetchList(); setOpenEdit(false); }}
                />
            </SidebarInset>
        </SidebarProvider>
    );

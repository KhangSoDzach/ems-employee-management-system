...
function AssetCard({
  asset,
  onReportIssue,
}: Readonly<{ asset: MyAsset; onReportIssue: (asset: MyAsset) => void }>) {
...
export default function MyAssetsPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!incidentType) {
      newErrors.incidentType = SYSTEM_MESSAGES.MY_ASSETS.TOAST_SELECT_TYPE;
    }
    if (!description || description.trim().length < 10) {
      newErrors.description = SYSTEM_MESSAGES.MY_ASSETS.TOAST_DESC_MIN;
    }
    ...
    setSubmitting(true);
    try {
      await assetService.submitReport(
        selectedAsset.id,
        {
          incidentType,
          description,
      ...
      toast.error(SYSTEM_MESSAGES.MY_ASSETS.TOAST_FAILED, {
        description:
          ...
          SYSTEM_MESSAGES.MY_ASSETS.TOAST_ERROR_DEFAULT,
      });
    } finally {
      setSubmitting(false);
    }
        ...
        <DialogContent className="sm:max-w-[480px] p-0 gap-0 rounded-xl overflow-hidden">
          <div className="px-6 pt-6 pb-4 border-b border-border">
            <DialogHeader>
              <DialogTitle className="text-lg font-bold text-foreground">
                {SYSTEM_MESSAGES.MY_ASSETS.REPORT_TITLE}
                {selectedAsset
                  ? `${SYSTEM_MESSAGES.SYMBOLS.DASH}${selectedAsset.name}`
                  : ""}
              </DialogTitle>
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              >
                <SelectTrigger
                  className={`w-full h-9 text-sm ${errors.incidentType ? "border-red-500 focus:ring-red-500" : ""}`}
                >
                  <SelectValue
                    placeholder={SYSTEM_MESSAGES.MY_ASSETS.PLACEHOLDER_INCIDENT}
                  />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                >
                  {SYSTEM_MESSAGES.MY_ASSETS.LABEL_REQUIRED}
                </span>
              </div>
              <Textarea
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-foreground">
                {SYSTEM_MESSAGES.MY_ASSETS.LABEL_ATTACHMENT}
              </label>
                  ...
                  {attachment ? (
                    <>
                      ...
                      <span className="text-[11px] text-muted-foreground">
                        ...
                        {SYSTEM_MESSAGES.MY_ASSETS.UNIT_MB}
                      </span>
                    </>
                  ) : (
                    <>
                      <span className="text-sm font-medium text-foreground">
                        {SYSTEM_MESSAGES.MY_ASSETS.NO_FILE}
                      </span>
                      <span className="text-[11px] text-muted-foreground">
                        {SYSTEM_MESSAGES.MY_ASSETS.FILE_LIMIT}
                      </span>
                    </>
                  )}
                </div>

          ...
          <div className="space-y-6">
            ...
            <div className="content-card">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {/* Leave Balances */}
                {(leaveBalancesQuery.data || []).map(
                  (balance: any, index: number) => {
                    const type = balance.leaveType;
                    const config =
                      LEAVE_TYPE_STATS_CONFIG[type] ||
                      LEAVE_TYPE_STATS_CONFIG.DEFAULT;
                    const labelKey =
                      `TYPE_${type}` as keyof typeof SYSTEM_MESSAGES.LEAVE;
                    const rawLabel =
                      (SYSTEM_MESSAGES.LEAVE[labelKey] as string) || type;
